...
17. [GDPR, Data Lifecycle, and Compliance](#17-gdpr-data-lifecycle-and-compliance)
18. [Open Questions and Sign-off Register](#18-open-questions-and-sign-off-register)
19. [Appendix A — Error Taxonomy](#19-appendix-a--error-taxonomy)
20. [Appendix B — Glossary](#20-appendix-b--glossary)
21. [Appendix C — Operational Runbooks](#21-appendix-c--operational-runbooks)
...
Where this document and `org_membership_lld_5.md` disagree on an invariant carried over unchanged from that source LLD, the source LLD is authoritative for the invariant's original rationale; this document's job is to state clearly which invariants transferred unchanged (§4.2) versus which required a substantive new design decision at extraction time (§12's decision register). Where this document and the shipped `iam-catalog-admin` implementation disagree, a code-vs-LLD reconciliation pass corrects the document (see Revision History above; **CAT-D8**, §12, is one such correction) rather than letting the two silently drift apart.

### 1.1 Relationship to source documents

| Source | What it specifies | Where this LLD refines it |
|---|---|---|
| ADR-0007 | The decision to extract `departments`/`plans`/`tender_acl` into three Wave services | §1, §16 |
| Document 1 (`01-hld-delta-decomposition.md`) §2.1 | "Stays in Core" vs. "moves out" table | §2 |
| Document 1 §4.1 | The `tenant_feature_flags` naming note — why the per-tenant override delta stays in Core | §2.2, §4.6, §12 (CAT-D2) |
| Document 1 §11 | Cross-service migration sequencing (Wave 1) | §16 |
| Document 1 §12 | Cross-service risk register | §6, §12 (CAT-D7) |
...
| `iam-catalog-admin` (shipped implementation) | Ground truth once code exists — divergences tracked in the Revision History and §12 | Throughout, post-v1.1 |
...
`⊕` is the same flat, per-key, override-wins, scalar-only merge as before (PLAN-6(d)) — nested values are still rejected, still at write time (this service rejects a non-scalar `feature_set` value the same way Core rejects a non-scalar `feature_flags` override). **This service owns the left operand only.** Document 1 §4.1 records why the ADR's "`tenant_feature_flags`" shorthand does not imply a third table belongs here. §4.6 restates this boundary against the full data-ownership table; §12 (CAT-D2) records the decision not to relocate the override delta here.
...
│   │   │   └── errors.go                  # sentinel errors, DomainError, NewError, WithDetails (§19)
...
│                                          #   e2e/ (full-stack HTTP), fixtures/ — no test/integration/ tier at
│                                          #   all (§15): this service has no events/outbox to cross-service-test
...
Per the O&M LLD's own recommendation, the shipped schema uses **nullable limit columns with `NULL = unlimited`** rather than a `-1` sentinel (the LLD's §4.2 note flags this as the recommended final choice) — `workflow_template_limit`/`tender_limit` are `NULL` for Enterprise's unlimited tiers, `>= 0` otherwise (**CAT-D6**, §12).
...
Applied on startup by `platform-pgcommon`'s `migrate.Runner` against the direct (non-PgBouncer) DSN (§3.3.2, §13) from `internal/adapter/outbound/postgres/migrations/`. As of v1.25, there is a single consolidated migration:
...
- **API version** in the path: every route is under `/api/v1`, including the mesh-only internal routes (**CAT-D8**, §12) — there is no separate prefix-free `/internal/*` namespace.
...
- The service **trusts the gateway-injected headers** `x-user-id`, `x-tenant-id` (mTLS guarantees they came from the gateway, matching every sibling IAM service's contract) — there is no JWT parsing in this service; `IdentityBridgeMiddleware` only validates header presence/shape and parses `x-tenant-roles` into `rc.Roles`. `x-tenant-id` is required and logged on every request even though this service never consults the parsed value for anything (§10.1, **CAT-D12**, §12).
- Content type `application/json`, enforced by `RequireJSONContentType` ahead of every write route (`415 unsupported_media_type` otherwise, §19). Timestamps are RFC 3339 UTC. Request bodies are capped at 1 MB, checked twice (`Content-Length` pre-check plus `http.MaxBytesReader` for chunked requests, `413 request_entity_too_large` otherwise, §19).
- Errors use the platform-wide `gincommon.ErrorResponse` shape (§19).
...
**Internal routes (`/api/v1/internal/*`, CAT-I1, CAT-I2).** Not user-facing. Reachable only from in-mesh service callers (Core Org & Membership, Group Mapping Service) behind a NetworkPolicy allow-list; authorized by the `iam-system` role via `RequireSystemRole` plus an in-handler `requireSystem` re-check — never by path shape (**CAT-D8**, §12). No JWT parsing, no tenant context to check (§10.4).
...
The full machine-readable error taxonomy, including every code's HTTP status and any extra response field, is in §19.
...
**This service does not build a local stand-in for that mechanism (CAT-D10, §12).** CAT-1/CAT-2/CAT-5 writes today produce no durable, queryable audit record beyond structured request logging (`gincommon.ObservabilityMiddlewares`) — the same posture O&M's own operator writes have, since O&M's LLD also never specifies a durable store for the "audit entry" it references. Once the Audit Log Service's direct-write contract is actually specified, this service should call it the same way — that is the intended fix, tracked as an open item (§18), not resolved by this document inventing a one-off table against a contract that doesn't yet exist.
...
None of these figures existed in the source LLD for the operator-facing endpoints — O-1/O-2/O-5/O-6 carried no SLO row there either. They are this document's own proposal, sized by analogy to the source LLD's simplest single-row read/write SLOs, and are tracked as an open confirmation item (**CAT-Q5**, §18) rather than presented as already agreed.
...
- `catalog_admin_request_duration_seconds{route, quantile}` — feeds the §11.1 SLOs directly.
...
Pointers only here; full procedures are in Appendix C (§21): recovering from a stalled `410 Gone` migration soak, forcing a consumer cache refresh ahead of its TTL, and rolling back a Wave-1 cutover before the Contract step.
...
catalogAdminConfig:
  ...
  migration:
    wave1SoakMinReleases: 1       # minimum releases the 410 Gone soak runs before the Contract step (§16 step 3) — see CAT-Q2, §18
  featureFlags:
...
Runs in the `iam` namespace alongside its sibling services, at 2 replicas (§11's "sufficient for MVP load" judgment, tracked as an open confirmation item, **CAT-Q5**, §18), behind a Pod Disruption Budget. No public ingress — every route is either `platform_operator`-gated operator tooling or a mesh-internal, `iam-system`-role-gated `/api/v1/internal/*` call (§10); no Envoy route terminates here for ordinary tenant traffic beyond CAT-6/CAT-7's own authenticated-caller reads.

### 14.2 Repository and package layout
...
- **Behavior-preservation contract** (specific to this service's extraction claim): a snapshot comparison of `planDefaults(plan)`'s output — computed against Core's pre-cutover local `plans` table and against this service's post-seed `plans` table — asserted byte-identical for all three tiers, directly testing the claim in §4.7/§16 step 2 rather than trusting the seed data was transcribed correctly by inspection alone. Run once, in staging, as a migration-verification gate before the Wave-1 write cutover (§16 step 3) proceeds.
...
Every migration in §16 is tested for forward-only application against a snapshot of the previous schema version. The Wave-1 cutover's four stages are each tested independently in staging (per §14.4's isolated staging environment) before being run against production: expand (schema created, seed data matches Core's exported rows exactly), cut-over-reads (Core's/Group Mapping's cache-population code produces identical cached values pre/post, per §15.3's behavior-preservation contract), cut-over-writes (a write to a retired O-1/O-2/O-5/O-6 route returns `410`, never silently succeeds against Core's now-unreplicated copy), and contract (the FK-to-app-check conversion is verified against a deliberately stale cache to confirm the documented fail-safe behavior in §9, before Core's tables are actually dropped).

### 15.6 Chaos and failure-injection tests
...
### 16.2 Zero-downtime requirements
...
## 17. GDPR, Data Lifecycle, and Compliance

### 17.1 What PII actually passes through this service
...
### 17.2 This service's role in tenant offboarding and GDPR erasure
...
### 17.4 Retention schedule summary
...
No maintenance/pruning job exists for this service (§16.4) — there is no compliance-relevant retention window this document needs to reconcile against a pruning schedule, since nothing here is ever deleted except by an explicit, rare, operator-driven action.

---

## 18. Open Questions and Sign-off Register

Following the same classification convention as the sibling LLDs (Groups **A** — cross-document amendments requiring another document's sign-off, **B** — product/cross-team decisions to confirm, **C** — resolved, recorded for traceability, **D** — deferred, not blocking implementation), with items under the **CAT-Q#** prefix — distinct from this document's own decision register (**CAT-D#**, §12, which records decisions this document has already made) and from the source LLD's `A#`/`B#`/`C#`/`D#` numbering. None of the items below blocks this document's "Approved for implementation" status — each states explicitly what, if anything, it is still waiting on.

### Group A — Cross-document amendments

**CAT-Q3 — ADR-0007 Action Item 6's "full `NOT VALID`→drop sequence" is referenced by Document 1 but not written out at that level of mechanical detail here.** *(§16)* Document 1's own risk register (§12 of that document) tracks "Full `NOT VALID`→drop sequence" for the FK losses (`fk_tenants_plan`, `fk_td_department`, `fk_gdm_department`) as ADR-0007 Action Item 6. This document's §16 describes the FK-to-app-check conversion at the ordering level (convert before drop) but does not itself reproduce `NOT VALID`/`VALIDATE`-style mechanics, since the conversion targets are being *dropped*, not validated in place — there is no Postgres constraint left to `VALIDATE` on Core's side once the FK is gone. **Status: Overtaken by implementation, sign-off still open (updated v1.23)** — §16's Contract step has already executed using exactly this mechanism ("convert to app-level check, then drop"), so this is no longer a precondition blocking anything; what remains open is purely a documentation/traceability question — confirmation from whoever owns Action Item 6 that this was what it referred to, and that no separate mechanical write-up is still expected. Recorded here rather than silently closed, since that confirmation was never actually obtained before implementation proceeded.

### Group B — Product / cross-team decisions to confirm

**CAT-Q1 — Consumer-side cache TTLs are this document's proposal, not a confirmed commitment from the teams that implement them.** *(§6)* The 600 s TTL for `om:departments`/`om:plans`/`gm:departments` and the 24 h stale-if-error TTL are design choices this LLD makes on behalf of code that lives in Core's and Group Mapping Service's own repositories, not this service's. **Status: Overtaken by implementation (updated v1.23)** — this item's own gate ("before their cache-population code is implemented") has already passed: §16 step 2 states Core's/Group Mapping's cache-population code has been implemented and is live, using exactly these TTLs. No record of the explicit confirmation this item sought exists; production behavior at these TTLs is the de facto confirmation, but that is different from the owning teams having actually signed off. Recorded here for traceability rather than silently marked resolved, the same way this document's own convention treats every other case where implementation outran a documented precondition.

**CAT-Q2 — Wave-1 `410 Gone` soak-period duration is not given a concrete length.** *(§16 step 3)* §16 states that Core's retired handlers return `410 Gone` "for one release" before removal, but does not state a minimum wall-clock duration (days/weeks) or a monitoring-confirmed exit criterion beyond "any caller that missed the tooling cutover." **Status: Overtaken by implementation (updated v1.23)** — the step this item gates ("before the Contract step is scheduled") has already run: §16 step 4 is marked "done," and step 3's own text confirms the soak "already ran to completion" before the retired handlers were removed. Whatever duration was actually used was never captured as a concrete number here; nothing further to schedule, but the gap between "no concrete duration was ever agreed" and "the soak ran anyway" is recorded rather than erased.
...
Every code in this table sets the response body's `error` and `code` fields to the same value. `duplicate_code` (and `invalid_uuid`, `parseUUIDParam`'s own validation code, not listed here since it's a shared helper rather than an endpoint-specific error) briefly did not: a handler attaches a more specific sub-code via `WithDetails({"code": ...})` on top of a generic sentinel (`conflict`, `validation_error`), and the shared error-envelope builder (`errorResponseWithDetails`) previously set `error` from the sentinel before that override was merged, never revisiting it — so `error` stayed on the generic sentinel while `code` carried the specific one. Fixed by having `errorResponseWithDetails` re-sync `error` to the final, post-override `code` (`internal/adapter/inbound/http/middleware.go`), so every sub-code, present or future, inherits the guarantee rather than needing a per-call-site fix. See resolved decision **CAT-D9** (§12).

*(Residual documentation note, v1.24: `router.go`'s body-cap check carries a code comment citing "LLD §20" — this document's error-taxonomy section number before this revision's restructure, now §19. Left as-is since fixing it is a code-comment change, out of scope for a markdown-only revision; noted here so the citation isn't mistaken for a fresh discrepancy by a future reconciliation pass.)*
...
- **`planDefaults(plan)`** — the function, owned by Core, that resolves a tenant's plan tier to its baseline entitlements; as of this extraction it reads this service's `plans` row (via `om:plans`) instead of a local `SELECT` or hardcoded map (§5.4, §16 step 2).
...
- **`record_version`** — the optimistic-locking column on both tables; a CAT-2/CAT-5 write must supply the current value or receive `409 optimistic_lock_conflict` (§5, §9.1, §19).
...
## 21. Appendix C — Operational Runbooks

### C.1 Recovering a stalled Wave-1 `410 Gone` migration soak *(historical — window closed, updated v1.23)*

**This runbook's window has closed.** §16's Contract step has completed and the `410`-returning routes it would monitor no longer exist in Core at all (removed, not merely retired) — the alert this runbook's trigger names is itself labeled "moot" in §11.5, for the same reason. Retained below only as the historical record of the procedure that would have applied during the soak, not as a still-executable runbook; previous revisions left this framed as live, contradicting §16/§11.5's own "completed"/"moot" status — corrected here rather than left as an active-sounding procedure for a window that no longer exists.

Trigger (historical): the §11.5 "`410 Gone` traffic on the pre-drop retirement routes" alert firing repeatedly past the agreed soak duration (**CAT-Q2**, §18). Procedure (historical): (1) inspect the `410` traffic's source (request logs/tracing, §11.3) to identify the caller that missed the tooling cutover in §16 step 2; (2) confirm with that caller's owning team whether it is a known, in-progress migration or a genuinely missed one; (3) if missed, repoint that caller at the new CAT-1 through CAT-7 endpoints and confirm `410` traffic drops to zero before re-evaluating the soak's exit criterion; (4) do not proceed to the Contract step (§16 step 4) while any `410` traffic remains, regardless of how long the soak has already run.
...
**This rollback path no longer exists.** §16's Contract step has already executed (Core's tables and pre-Contract handlers are gone), and this runbook's own step (3) already says as much ("this rollback path does not exist once Contract has executed") — but the trigger and procedure below were still worded as an active option, contradicting that. Retained purely as the historical record of what rollback looked like before Contract ran, not as a currently available procedure; if a defect is found in Catalog Service's behavior today, recovery is via §16's actual (heavier) post-Contract path — restoring Core's tables from the pre-drop snapshot/replication stream and replaying any Catalog-Service-only writes since cutover — not via the steps below.

Trigger (historical): a defect discovered in Catalog Service's own behavior after the read or write cutover (§16 steps 2–3) but before the Contract step (§16 step 4) had run. Procedure (historical): (1) repoint operator tooling and Core's/Group Mapping's cache-population code back at Core's original endpoints/tables, which remained live and authoritative until Contract ran (§16's own rollback note); (2) confirm Core's O-1/O-2/O-3/O-5/O-6 handlers were still serving correctly (they only returned `410` post-write-cutover, never removed before Contract); (3) file the defect against this service before attempting cutover again — this rollback path does not exist once Contract has executed (§16's rollback note: "requires restoring Core's tables from the pre-drop snapshot").

### C.4 Catalog database credential rotation

This service has no third-party credential (§13) — only connection-string secrets: `DATABASE_URL`, `MIGRATION_DATABASE_URL`, and `VALKEY_URL` (corrected v1.23 to include the latter two, previously omitted here and in §13). Rotation follows the platform's standard External Secrets Operator rotation hook identically to every sibling service; no service-specific runbook step exists beyond confirming (via §11.4's dashboards) at least one successful query using the new Postgres credential, plus — for `VALKEY_URL` specifically — a `cat:departments`/`cat:plans` cache-hit-ratio panel (§11.2's PromQL derivation) returning non-zero shortly after rotation, before considering rotation complete.

---

*End of document. This LLD refines Document 1 (`01-hld-delta-decomposition.md`) and `org_membership_lld_5.md`'s `departments`/`plans` sections, and is the input to implementation of the `iam-catalog-admin` service. §1.1 records where each source document is refined; §18 tracks every remaining open item.*
