...
│           │   └── migrations/            # 000001–000004 (up/down .sql, §4.4)
...
Applied on startup by `platform-pgcommon`'s `migrate.Runner` against the direct (non-PgBouncer) DSN (§3.3.2, §13) from `internal/adapter/outbound/postgres/migrations/`. This document names only the migrations its own revision history and other sections independently reference — it does not claim a full enumeration beyond what has been individually verified elsewhere in this text:

| Migration | Purpose |
|---|---|
| `000001_init_schema` | Baseline schema — both ENUM types (§4.1), the `departments`/`plans` tables (§4.2) and their triggers (§4.5), the two indexes. Confirmed byte-identical against the shipped file as of the v1.15 and v1.23 audits. |
| `000002` | Not independently named or verified elsewhere in this document — omitted here rather than guessed at, per this document's own no-fabrication convention (see the Revision History's audit trail for what *has* been checked). |
| `000003_roles_grants` | Creates the single `catalog_admin_app` role and its grants (§10.3) — deliberately no migrator/app-role split, per that migration's own comment ("no cross-tenant data exists to read around"). |
| `000004_department_name_immutable_errcode` | Adds `ERRCODE = 'check_violation'` (SQLSTATE 23514) plus a synthetic `CONSTRAINT`/`DETAIL` to the `prevent_system_department_name_change` trigger (§4.5), so `pgcommon.ConstraintName(err)` resolves it structurally instead of by message substring (v1.14). |

A short-lived `000004_audit_log`/`000005_audit_log_grants` pair existed between v1.9 and v1.10 and was fully reverted (§12, CAT-D10) — not part of the current migration set, mentioned here only so a reader cross-checking the revision history against a live migrations directory does not conclude one is missing.
...
| `prevent_system_department_name_change` | `departments` | `BEFORE UPDATE` | Rejects a `name` change while `is_system = true` (D-11), raising `422 system_name_immutable` via `pgcommon.ConstraintName` once `000004`'s SQLSTATE/constraint metadata was added (§4.4). |
...
Not run as a separate quarterly exercise, unlike a service with an external stateful dependency (Keycloak, an event bus) — this service's entire failure surface is already covered by §9.3's failure matrix and its own straightforward causes (DB down, pod down, cache miss). The one scenario worth deliberately injecting: a CAT-I1/CAT-I2 call failure coinciding with a consumer's primary cache **and** stale-if-error key both being empty (a brand-new Core/Group-Mapping replica's very first request, §9.3) — verified as part of Core's/Group Mapping's own chaos-testing surface, not this service's, since the assertion being tested (does the caller correctly return `503 catalog_service_unavailable` rather than a wrong answer) lives entirely in the caller's code.
...
| `catalog_service_unavailable` | 503 | — | Emitted by a **consumer** (Core, Group Mapping), not this service — included here because it is the direct downstream consequence of this service's own unavailability coinciding with an empty primary-and-stale-if-error cache (§9.3); a reader of this appendix should not expect to see it in this service's own logs. |
