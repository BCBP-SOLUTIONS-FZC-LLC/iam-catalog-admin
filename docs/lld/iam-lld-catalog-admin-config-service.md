...
| 1.24 | 2026-08-22 | **Structural realignment to match `iam-lld-user-profile.md`'s section template — no functional/contract change.** Renumbered and regrouped this document's sections to follow that sibling LLD's core outline (§1 Document Overview, §2 Service Responsibilities and Boundaries, §3 Architecture and Package Layout, §4 Data Model, §5 API Contract, §6 Caching Design, §7 Event Architecture, §8 Key Request Flows, §9 Concurrency/Consistency/Failure Handling, §10 Security, §11 Observability), with this service's own extra sections — none of which the smaller `iam-user-profile` needs — retained as later numbered sections in the same style: §12 Decision Register (renamed from "New decision register entries" — a standing register, not just "new" ones), §13 Configuration, §14 Deployment and Scaling, §15 Testing Strategy, §16 Migration Plan, §17 GDPR/Data Lifecycle/Compliance, §18 Open Questions and Sign-off Register, §19–§21 Appendices A–C. **New material added to reach the template's density, all directly derivable from facts this document already stated (no invented requirements, endpoints, or behavior):** a shared request-preamble subsection (§8.4.0, mirroring the template's §8.8.0) factoring out the identity/role-check middleware chain every route already ran through per §10; seven new per-endpoint sequence diagrams (CAT-1, CAT-3, CAT-4, CAT-5, CAT-6, CAT-7, CAT-I2 — CAT-2's write diagram and the CAT-6/CAT-I1 cache-mechanism flowchart already existed and are retained, at §8.4.2 and §8.3 respectively) so every catalogued endpoint has one, matching the template's one-diagram-per-endpoint convention; explicit `4.3 Row-Level Security` / `4.4 Migrations` / `4.5 Triggers` subsections under Data Model — §4.3 states plainly this service has none (§10 already said so; now Data Model says so too, in the slot the template reserves for it); §4.4/§4.5 enumerate only the migrations/triggers this document's own text already names elsewhere (`000001_init_schema`, `000003_roles_grants`, `000004_department_name_immutable_errcode`; `touch_row`, `prevent_department_delete`, `prevent_department_code_change`, `prevent_system_department_name_change`) rather than fabricating a full migration history this document has never independently verified end-to-end. §5's endpoint catalogue reformatted to the template's `# | Method & path | Purpose | AuthZ | Cached` column shape (same nine rows, same facts). §9.1/§9.2 (Optimistic concurrency, Transaction discipline) are new subsections synthesizing facts already stated at the old §6/§20 (the `record_version` field, `409 optimistic_lock_conflict`) and §4.3.2 (`RunInTx`/`withPool`) — no new claim. **Nothing about this service's behavior, contract, schema, or operational posture changed.** Every decision-register entry (CAT-D1–CAT-D12), open item (CAT-Q1–CAT-Q7), error-taxonomy row, requirement ID (CAT-1–7, CAT-I1/I2, CAT-FAIL-1–4, CAT-EVT-1–5), and revision-history entry (v1.0–v1.23, above) is unchanged in substance and locatable in the renumbered document via the mapping table immediately below the Table of Contents. **§-references inside revision-history entries dated v1.0–v1.23 above are deliberately NOT retroactively renumbered** — each cites the section numbers that were current on the date it was written, which is what makes it a historical record rather than a live cross-reference; use the mapping table to resolve an old citation to its current location. No code changed — this revision is markdown-only. |
| 1.25 | 2026-08-25 | **Postgres migrations consolidated from four files into one — pre-production housekeeping, no functional/contract change.** This service has still never been deployed to any environment, so no environment holds an earlier multi-file migration history that needs preserving; `000001_init_schema`, `000002_triggers`, `000003_roles_grants`, and `000004_department_name_immutable_errcode` were merged into a single `000001_init_schema.up.sql`/`.down.sql` containing the schema, seed data, all four lifecycle triggers (including `prevent_system_department_name_change`'s structured `ERRCODE = 'check_violation'`/synthetic `CONSTRAINT` — previously added by the separate `000004` migration, now present from the start), and the `catalog_admin_app` role/grants, in that order. CHANGELOG.md's `[Unreleased]` section was also trimmed to concise bullets in the same pass — no content lost, just condensed (the fuller narrative remains in this document's own revision history and in git history). **§4.4/§4.5 updated** to describe the single consolidated migration instead of four; the package-layout diagram's `migrations/` line and §10.3's citation of `000003_roles_grants.up.sql`'s own rationale comment (now `000001_init_schema.up.sql`) updated to match. Verified: `go build`/`go vet ./...`, a full unit + `-tags=integration` Postgres (via testcontainers) + `-tags=e2e` full-stack (real Postgres+Valkey) run, all green against the consolidated migration. `ARCHITECTURE.md`, `README.md`, and `.claude/*.md` reconciled in the same pass — see their own edit history for specifics (this document doesn't duplicate sibling-doc changelogs). |
| 1.26 | 2026-08-25 | **`test/postgres`/`test/e2e` parallelized — test infrastructure only, no functional/contract change.** All 240 top-level test functions across both packages spin up their own fully isolated Postgres testcontainer (e2e additionally its own `miniredis` instance and `httptest.Server`), but none called `t.Parallel()`, so Go ran them one at a time per package and paid the ~1s-per-test container-boot cost serially — the dominant cost in both suites' wall time. Added `t.Parallel()` to every one of them; verified safe first (no package-level shared state, no fixed ports, no `os.Setenv`, no `t.Run` subtests in either package). Measured on a 10-CPU machine: `test/postgres` 31s → 8s, `test/e2e` 220s → 50s, full `make test-ci` (with `-race`) ≈73s. **§15.2** gained a sentence noting the integration tier runs its tests concurrently. No test assertions, coverage instrumentation, or CI gate behavior changed — `make test-ci`'s merged-coverage output and the ≥98% gate are unaffected (currently 99.6%, per v1.25's own verification). `README.md` and `.claude/CLAUDE.md`/`development-guide.md` updated with the same facts and a troubleshooting note for future tests that forget `t.Parallel()`. |
| 1.27 | 2026-08-25 | **Four historical/superseded root-level docs deleted — documentation-only, no functional/contract change.** `MIGRATION_RUNBOOK.md` (Wave-1 rollout phases — the migration is complete, per the v1.13 correction and §12/§16 of this document), `IMPLEMENTATION_GAP_ANALYSIS.md` (the pre-implementation LLD-vs-O&M gap analysis that drove the original extraction), `EVENT_COMPATIBILITY_REPORT.md` (the point-in-time no-events compatibility check — the underlying fact is restated directly in `ARCHITECTURE.md`'s "Event architecture" section and `.claude/api-and-events.md`), and `O_AND_M_DELTA.md` (the O&M-side integration delta — already applied on that side, per CHANGELOG.md's cutover-complete entry) were all point-in-time artifacts whose content is either fully superseded (the rollout they tracked has finished) or already duplicated in a living document (this LLD's own §12 Migration Plan and §14 Decision Register, and the CHANGELOG). Kept as a durable record only in this document and CHANGELOG.md, rather than four one-off, ungoverned files a future reconciliation pass would otherwise have to keep in sync by hand. **§3's package-layout diagram** and **CAT-D8's (§14) corroborating citation** updated to drop the removed filenames; every cross-reference to them elsewhere in the repository (`ARCHITECTURE.md`, `CACHE_DESIGN.md`, `README.md`, `.claude/CLAUDE.md`/`api-and-events.md`/`flows-and-concurrency.md`) removed or reworded in the same pass — this document's own v1.0–v1.26 revision-history entries that cite these filenames (e.g. v1.13) are left untouched, per this document's own no-retroactive-editing convention: they describe what was true on the date each was written. No code changed. |
| 1.28 | 2026-08-26 | **Two real code fixes found via a fresh shared-library-usage audit (prompted by "check all logs/metrics/traces pass through platform-gincommon" and "all database connection/config/operations pass through platform-pgcommon"), both bringing this service back in line with `iam-org-membership`/`iam-user-profile`'s own patterns. (1) `main.go`'s Postgres DSN assembly applied `ApplyStatementTimeout` unconditionally to `pgcommon.ConfigFromEnv()`'s DSN, including when `DATABASE_URL` was set. `DATABASE_URL` is returned verbatim by `pgcommon` (§9's own prose already said this) and may have no `?` query string of its own, so a `DATABASE_URL` set without one, combined with `PG_STATEMENT_TIMEOUT`, produced a malformed DSN (a stray `db&options=...` instead of `db?options=...`) — untested, since `dsn_test.go` only ever exercised `ApplyStatementTimeout` against a bare DSN, never the `DATABASE_URL`-set case. Fixed by adding `postgres.DSNFromEnv()` (`db.go`) — the same name and behavior as the identical helper already shipping in both sibling repos: skip the `ApplyStatementTimeout` append when `DATABASE_URL` is set, apply it otherwise. `main.go` now calls `pgadapter.DSNFromEnv()` instead of `pgadapter.ApplyStatementTimeout(pgCfg.DSN)` directly. Note this reintroduces a function named `DSNFromEnv` that v1.14's own entry (above) recorded as deliberately removed in favor of a bare `ApplyStatementTimeout` call — that removal is not being reversed in spirit; v1.14 removed a *different*, larger `DSNFromEnv` that hand-assembled the DSN itself (`envOrDB`) before `pgcommon.ConfigFromEnv` existed to do that job. This new `DSNFromEnv` is a thin wrapper *around* `pgcommon.ConfigFromEnv`, only adding the `DATABASE_URL` guard — it does not reintroduce the hand-rolled parsing v1.14 removed. Added `TestDSNFromEnv_DATABASE_URL_TakesPrecedence` (`dsn_test.go`), mirroring `iam-user-profile`'s own regression test of the same name. (2) `newErrorResponse` (`internal/adapter/inbound/http/errors.go`) read the response's `trace_id` field via the raw `go.opentelemetry.io/otel/trace` API (`trace.SpanFromContext(...).SpanContext().TraceID()`) instead of `gincommon.TraceIDFromContext(c)` — the helper `HandleError` (`middleware.go`) already uses for the identical field, and the one `platform-gincommon`'s own tracing middleware actually populates (`c.Set("trace_id", ...)` into the Gin context, per `TraceIDFromContext`'s implementation) — an inconsistency v1.11's entry (above) came close to surfacing (it added the `gincommon` helpers to `middleware.go` specifically) but never checked `errors.go` against. Fixed to call `gincommon.TraceIDFromContext(c)`, matching `middleware.go`; `otel/sdk`/`otel/trace` are no longer direct imports anywhere in this repo (moved to `// indirect` in `go.mod` via `go mod tidy`, pulled in transitively by `platform-gincommon` only). `errors_test.go`'s `TestNewErrorResponse_TraceIDFromSpan` (which fabricated a standalone `sdktrace.NewTracerProvider()` span) replaced with `TestNewErrorResponse_TraceIDFromGinContext`, which stamps `c.Set("trace_id", ...)` directly — the same pattern `middleware_test.go` already used for the equivalent `HandleError` assertion. Both fixes are internal-only; no wire-contract, schema, or observable-behavior change (the DSN bug was latent — no environment has set `DATABASE_URL` without a query string in combination with `PG_STATEMENT_TIMEOUT` yet — and the trace_id fix produces the identical value in the one path that was already covered by a live Gin request going through `TracingMiddleware`). §4.3.2's `pgcommon` integration paragraph and the package-layout diagram (§3) updated to name `DSNFromEnv`. Verified: `go build`/`go vet ./...`, `gofmt -l .` clean, full unit suite green. |
| 1.29 | 2026-09-09 | **Shared-library observability alignment with `iam-org-membership`/`iam-realm-provisioner`** (prompted by "check all logs/metrics/traces pass through platform-gincommon"). Three real gaps vs those siblings, all in `cmd/catalog-admin-config/main.go` and the postgres adapter: (1) **`gincommon.InitTracingFromEnv()` was gated on `OTEL_EXPORTER_OTLP_ENDPOINT`**, so in-process spans had no valid trace IDs in local/dev when no collector was configured — siblings always install the TracerProvider (OTLP export is already a no-op until the endpoint is set). Ungated. (2) **`ObservabilityMiddlewares` was only applied inside `NewRouter`**, after the pool was built, so `pgmetrics` never ran and `catalog_admin_*` registration happened after router construction rather than as gincommon's documented metrics-init API. Primed in `main.go` before any collector registration (same `sync.Once` as the later router apply). (3) **No `pgmetrics.InitWithRegisterer` and no `pgCfg.Tracer`**, so pool/query metrics and `db.query` spans were missing from the gincommon pipeline. Added `pgmetrics.InitWithRegisterer` onto `gincommon.MetricsRegisterer()` and `pgadapter.NewOTelTracer` (`internal/adapter/outbound/postgres/otel_tracer.go`) — the same thin adapter both siblings ship, wrapping gincommon's global TracerProvider for `pgcommon.Config.Tracer`. Response `trace_id` still comes only through `gincommon.TraceIDFromContext` (v1.28); the new OTel import is that tracer adapter only. e2e harness and docs (`ARCHITECTURE.md`, `README.md`, `.claude/*`, this §3.3/§11) updated to match. No wire-contract change. |
| 1.30 | 2026-09-09 | **Shared-library Postgres alignment with `iam-org-membership`/`iam-realm-provisioner`** (prompted by "all database connection/configuration/operation pass through pgcommon"). Bumped `platform-pgcommon` **v1.2.1 → v1.3.0** so this service can use `IsConnectionException`/`IsInsufficientResources`/`IsPgError` instead of inspecting `*pgconn.PgError` with `errors.As`. Four code changes, all matching the siblings' postgres adapter: (1) **`wrapConnErr` remaps SQLSTATE class 08/53/57/58 to `domain.ErrDBUnavailable`** via those pgcommon helpers (57/58 have no dedicated helper — classified from the `(SQLSTATE 57…)` / `(SQLSTATE 58…)` text `pgconn` puts in `Error()`), so production HTTP never type-asserts a raw PgError. Non-availability PgErrors still pass through for service-layer `IsUniqueViolation`/`IsCheckViolation` matching. (2) **Inbound `HandleError` dropped its `pgconn` import** — the leaked-PgError 503 fallback now uses the same helpers as wrapConnErr (`isOperatorOrSystemErrorSQLState` duplicated in the HTTP package so inbound never imports the outbound postgres adapter). (3) **`MigrationDSNFromEnv()` takes no `appDSN` argument**, applying `ApplyStatementTimeout` to `MIGRATION_DATABASE_URL` itself and falling back to `DSNFromEnv()` — the previous `(appDSN)` form skipped timeout on the migration URL. (4) **`ApplyStatementTimeout` is a no-op on empty DSN and already-timeout DSNs** (idempotent). Deliberately **not** copied from the siblings: `GUCProvider`/`SystemPoolConfig`/`TxRunner` — this service has no RLS and no multi-statement/outbox write (CAT-FAIL-3, §4.3, §7). §3.1 pin, §3.3 mermaid, §3.3.2, and §19 `db_unavailable` updated. No wire-contract change. |
| 1.31 | 2026-09-09 | **Test harness Postgres connections now go through pgcommon too** (same prompt as v1.30 — the leftover gap). `test/e2e` and `test/postgres` opened a second raw `pgxpool.New` beside the production `pgcommon.Pool` for trigger/assert SQL. Replaced with `test/dbseed` wrapping `pgcommon.NewPool`/`WithConn`, matching `iam-org-membership`/`iam-realm-provisioner`. Production path unchanged. |
| 1.32 | 2026-09-10 | **Structural realignment to match `iam-lld-org-membership-service.md`'s section template — no functional/contract change.** Renumbered and regrouped §12 onward to follow that sibling LLD's outline exactly, continuing the same "renumber the tail, keep §1–§11 stable" convention v1.24 used to align with `iam-lld-user-profile.md`'s template: §12 Configuration (was §13), §13 Deployment and Scaling (was §14), §14 Testing Strategy (was §15), §15 GDPR/Data Lifecycle/Compliance (was §17), §16 Open Questions and Sign-off Register (was §18) — this section now also houses the former standalone **§12 Decision Register** as its own "Decision Register" subsection immediately above Group A, since `iam-lld-org-membership-service.md` has no separate decision-register section of its own and this document's own Group C already treated the two as one register in substance ("that register *is* this document's Group C"); §17 Appendix — Error Taxonomy (was §19 Appendix A — Error Taxonomy, dropping the "A" to match the sibling's naming, since folding the Glossary and Runbooks out from under separate Appendix B/C letters leaves only one appendix); **§18 Integration Details (new)** — this service's integration surface restated from a dependency-table-and-per-consumer-subsection shape matching the sibling's §18, built entirely from facts this document already stated (§2, §5.4, §6, §9.3) rather than any new integration requirement — Core and Group Mapping are documented as callers of CAT-I1/CAT-I2, since this service itself has no outbound edge to describe (§3, §7); **§19 Migration Strategy (renamed from Migration Plan, was §16)** — same content, plus a new §19.5 "Historical runbook procedures (closed)" subsection folding in the former Appendix C §C.1/§C.3 runbooks (the closed `410 Gone` soak-recovery and pre-Contract rollback procedures), which overlapped substantially with this section's own already-stated rollback narrative; **§20 Operational Considerations (new)** — folds in the former Appendix C §C.2 (forcing a consumer cache refresh) and §C.4 (credential rotation) runbooks verbatim in substance, plus two short subsections synthesizing already-stated facts about `/readyz`'s dual-dependency check (§16 CAT-D11) and the optimistic-lock-conflict alert (§11.2/§11.5); **§21 Performance Considerations (new)** — restates the already-stated per-endpoint latency budgets (§5.4, §11.1) and cache-TTL sizing rationale (§6.1) in the sibling's own per-workload-class subsection shape, adding no new figure. The former **§20 Appendix B — Glossary** folds into a new **§1.2 Glossary** subsection of Document Overview (org-membership's own §1 has no glossary, but dropping the terms outright rather than relocating them would have made this document harder to read standalone, the same reasoning that justified keeping a Decision Register at all). A new Section renumbering map (v1.31 → v1.32) is added immediately below, mirroring the existing v1.23 → v1.24 map, to resolve any `§N` citation from a revision-history entry dated v1.0–v1.31 to its current location — per this document's own established convention (v1.24's own entry, above), revision-history entries dated v1.0–v1.31 are **not** retroactively renumbered; only live cross-references in the section bodies below were updated, together with the small number of `Document 1 §N`, `org_membership_lld_5.md §N`, `HLD §N`, and literal quoted code-comment citations (e.g. `"LLD §16 step 4"` in Core's own `main.go`) that were deliberately left untouched because they name a different document's section or a fixed historical string, not this document's own numbering. No code changed — markdown-only, verified by re-reading every renumbered section for a dangling or mismatched `§` citation after the pass. |
| 1.35 | 2026-09-10 | **Merged test coverage closed to 100%** (previously 99.9%, gate is ≥98%). `go tool cover -func` traced the entire remaining gap to a single statement: `postgres.DSNFromEnv`'s no-`DATABASE_URL` branch (the `PG_*`-vars path, which always calls `ApplyStatementTimeout`) had no test exercising it directly — every existing `DSNFromEnv` test set `DATABASE_URL`, hitting only the verbatim-return branch. Added `TestDSNFromEnv_NoDatabaseURL_AppliesStatementTimeout` (`dsn_test.go`), which unsets `DATABASE_URL`, sets `PG_USER`/`PG_DBNAME`/`PG_HOST`/`PG_STATEMENT_TIMEOUT`, and asserts the resulting DSN carries both the host and the appended `statement_timeout` option — a real regression test for the branch, not a coverage-only stub. No production code changed. Verified: `go build`/`go vet ./...` (default/`-tags=e2e`/`-tags=integration`), `gofmt -l .`, `go-arch-lint check`, full `make test-ci` (unit + postgres + e2e), merged coverage **100.0%**. `README.md`/`ARCHITECTURE.md`/`.claude/CLAUDE.md`'s coverage-percentage citations updated to match. |
| 1.34 | 2026-09-10 | **Third-party Go dependencies bumped to latest via `go get -u ./...` + `go mod tidy` — no functional/contract change, one real breaking change caught and fixed.** Direct bumps: `jackc/pgx/v5` v5.10.0 → v5.11.0, `redis/go-redis/v9` v9.8.0 → v9.22.0, `stretchr/testify` v1.11.1 → v1.12.1, `swaggo/swag` v1.8.12 → v1.16.6, `go.opentelemetry.io/otel` (+ `trace`/`metric`/`sdk`/`otlptrace`/`otlptracegrpc`) v1.45.0 → v1.46.0, plus dozens of indirect transitive bumps; both private shared libraries (`platform-gincommon`, `platform-pgcommon`) were already at their latest published tag (v1.3.0) and needed no bump. **Breaking change:** pgx v5.11.0 added `TypeMap() *pgtype.Map` to the `pgx.Rows` interface, breaking two hand-rolled test-only `pgx.Rows` implementations that predate this document's `test/dbseed` package (§3, added at v1.31) — `test/dbseed/pool.go`'s `bufferedRows` (fixed by returning its existing `m *pgtype.Map` field) and `internal/adapter/outbound/postgres/repo_whitebox_test.go`'s `mockRows` (fixed by returning a fresh `pgtype.NewMap()`, since that mock's `Scan` is a fixed sentinel-error stub that never touches the map). Neither production code path implements `pgx.Rows` itself (both repositories consume it via `pgxpool.Pool.Query`), so no production code changed. `docs/swagger/*` regenerated via `make swag` after the swag major-version jump and diffed by hand — all resulting changes confirmed benign (an `additionalProperties.format: int64` precision improvement on `record_versions`, a generated-header comment format tweak, and new library-internal fields on the `swag.Spec` struct literal in `docs.go` — no OpenAPI-surface change). Verified: `go build`/`go vet ./...` (plus `-tags=e2e`/`-tags=integration`), `gofmt -l .`, the full unit suite, `test/postgres` and `test/e2e` (both against real Docker-backed Postgres/Valkey, confirming pgx v5.11.0 and go-redis v9.22.0 work correctly at runtime and not just at compile time), `make vuln-check` (0 new findings), `go mod verify`, and `go-arch-lint`, all green; merged coverage **99.9%**, comfortably above the 98% gate. §1's Version/Date header and every other document's `LLD v1.33` cross-reference (`README.md`, `ARCHITECTURE.md`, `.claude/CLAUDE.md`) updated to `v1.34` in the same pass. |
| 1.33 | 2026-09-10 | **`catalog_admin_request_duration_seconds` converted from a Summary to a Histogram, reversing v1.6's original choice** — see new decision-register entry **CAT-D13** (§16) for the full reasoning. Prompted by a direct comparison against `iam-org-membership`'s `deploy/monitoring/slo-rules.yml` (multi-window multi-burn-rate SLO alerting): a Summary's client-side quantiles cannot compute a per-request good/bad-event ratio or be aggregated across routes with `histogram_quantile()`, both of which a genuine error-budget burn-rate SLO needs — the existing threshold-only `HighLatency`/`CriticalLatency` alerts (§11.5) don't require this, which is why the gap went unnoticed until this comparison. **Code:** `internal/adapter/outbound/metrics/metrics.go`'s `RequestDuration` is now a `*prometheus.HistogramVec` with buckets (`5/10/15/20/30/40/50/75/100/250/500/1000/2500` ms) chosen to land exactly on §11.1's own SLO thresholds; the `Observe()` call site (`router.go`'s `requestMetricsMiddleware`) is unchanged, since `prometheus.Observer` is satisfied by both metric types — a constructor-only change, verified via `go build`/`go vet ./...`/`gofmt -l .`/the full unit suite, all green, with no test needing a behavior change (`TestRegister_PreInitializesKnownLabels`, `TestRequestMetricsMiddleware_*` all pass unmodified). **New:** `deploy/monitoring/slo-rules.yml` — three route-class SLOs (public department reads/CAT-6-7 at 40 ms, internal bulk reads/CAT-I1-I2 at 30 ms, operator routes/CAT-1-2-4-5 at 100 ms, matching §11.1's own grouping exactly, including its choice to give CAT-4 and CAT-5 the same 100 ms target on their shared `/api/v1/operator/plans/:code` route rather than needing a method label to split them) with 5m/1h recording rules and fast/slow multi-window burn-rate alerts, mirroring `iam-org-membership`'s own file's structure but this service's own `critical`/`warning` severity vocabulary (§11.5) rather than that sibling's page/ticket labels. §11.2's metrics bullet and §11.5's alert table updated to match; `deploy/monitoring/dashboards/catalog-admin-requests-writes.json`'s latency panel changed from a single Summary-series query to three `histogram_quantile()` targets (p50/p90/p99). `ARCHITECTURE.md`, `README.md`, and `.claude/operations.md` updated to match — all three previously described this metric as a deliberate Summary; that description is now the change being reversed, not a remaining fact. Found while executing an unrelated `deploy/` parity pass against `iam-org-membership`'s own `deploy/` directory (which surfaced the missing `slo-rules.yml` in the first place) — this entry and CAT-D13 are the only load-bearing (code + contract-adjacent) outcome of that pass; the rest was Helm-chart parity (imagePullSecrets, a writable `/tmp` mount for `readOnlyRootFilesystem: true`, parameterized `podSecurityContext`/`securityContext`, `podAnnotations`/`podLabels`/`nodeSelector`/`tolerations` pass-through, `serviceAccount.name` override) and a stale-citation sweep across every `deploy/` file (many comments still cited pre-v1.24 LLD section numbers, two renumbering passes behind). |


---

## Section renumbering map (v1.23 → v1.24)

Use this table to resolve any `§N` citation from a revision-history entry dated v1.0–v1.23 (above) to where that content now lives. Live cross-references in the sections below this point already use v1.24 numbering.

| v1.23 § | v1.24 § | v1.23 § | v1.24 § |
|---|---|---|---|
| *(unnumbered)* Document Overview | §1 | §11 Failure handling | §9 |
| *(unnumbered)* Relationship to source documents | §1.1 | §11.1 Failure matrix | §9.3 |
| §1 Service Overview | §2 (intro) | §11.2 Failure invariants | §9.4 |
| §2 Responsibilities | §2.1 | §12 Migration plan | §16 |
| §3 Non-goals | §2.2 | §12.1–§12.4 | §16.1–§16.4 |
| §4 Architecture and Package Layout | §3 | §13 Observability | §11 |
| §4.1–§4.3.3 | §3.1–§3.3.3 | §13.1–§13.6 | §11.1–§11.6 |
| §5 Data Model | §4 | §14 New decision register entries | §12 |
| §5.1 `departments` DDL | §4.2 | §15 Configuration | §13 |
| §5.2 `plans` DDL | §4.2 | §16 Deployment and Scaling | §14 |
| §5.3 Ownership boundary | §4.6 | §16.1–§16.4 | §14.1–§14.4 |
| §5.4 Data ownership summary | §4.7 | §17 Testing Strategy | §15 |
| §5.5 ER diagram | §4.8 | §17.1–§17.6 | §15.1–§15.6 |
| §6 Endpoint Catalog (table) | §5.3 | §18 GDPR, Data Lifecycle, and Compliance | §17 |
| §6.1 Key request flows | §8.3 / §8.4 | §18.1–§18.4 | §17.1–§17.4 |
| §7 Internal APIs (§7.1/§7.2) | §5.4, §8.4.8/§8.4.9 | §19 Open Questions and Sign-off Register | §18 |
| §8 Cache architecture | §6 | §20 Appendix A — Error Taxonomy | §19 |
| §9 Security model | §10 | §21 Appendix B — Glossary | §20 |
| §10 Event Architecture | §7 | §22 Appendix C — Operational Runbooks | §21 |
| §10.1–§10.8 | §7.1/§7.3/§7.2/§7.4/§7.4.1/§7.5/§7.6/§7.7 *(reordered — see §7 intro)* | | |

---

## Section renumbering map (v1.31 → v1.32)

Use this table to resolve any `§N` citation from a revision-history entry dated v1.0–v1.31 (above) to where that content now lives. Live cross-references in the sections below this point already use v1.32 numbering. Sections 1–11 are unchanged in number (§1.2 Glossary is new content, not a renumbering); only §12 onward moved, per the v1.24-style realignment recorded in that revision's own entry above.

| v1.31 § | v1.32 § | v1.31 § | v1.32 § |
|---|---|---|---|
| §12 Decision Register | §16 (as the "Decision Register" subsection) | §17 GDPR, Data Lifecycle, and Compliance | §15 |
| §13 Configuration | §12 | §17.1–§17.4 | §15.1–§15.4 |
| §14 Deployment and Scaling | §13 | §18 Open Questions and Sign-off Register | §16 |
| §14.1–§14.4 | §13.1–§13.4 | §19 Appendix A — Error Taxonomy | §17 |
| §15 Testing Strategy | §14 | *(no v1.31 equivalent)* | §18 Integration Details (new) |
| §15.1–§15.6 | §14.1–§14.6 | §16 Migration Plan | §19 Migration Strategy |
| §16.1–§16.4 | §19.1–§19.4 | *(no v1.31 equivalent)* | §20 Operational Considerations (new) |
| §20 Appendix B — Glossary | §1.2 Glossary | §21 Appendix C — Operational Runbooks §C.2/§C.4 | §20.3/§20.5 |
| — | — | §21 Appendix C — Operational Runbooks §C.1/§C.3 | §19.5 (historical, closed) |
| — | — | *(no v1.31 equivalent)* | §21 Performance Considerations (new) |
...
The Catalog / Admin Config Service owns the platform's two **global, non-tenant-scoped reference catalogs** — `departments` and `plans` — that previously lived inside O&M's own database. It does **not** own the per-tenant override delta (`tenants.feature_flags`, stays in Core/O&M — §2.2, §4.6), pricing/billing, metering, or the I-8 hot-path entitlement merge (§2.2).

Where this document and `org_membership_lld_5.md` disagree on an invariant carried over unchanged from that source LLD, the source LLD is authoritative for the invariant's original rationale; this document's job is to state clearly which invariants transferred unchanged (§4.2) versus which required a substantive new design decision at extraction time (§16's decision register). Where this document and the shipped `iam-catalog-admin` implementation disagree, a code-vs-LLD reconciliation pass corrects the document (see Revision History above; **CAT-D8**, §16, is one such correction) rather than letting the two silently drift apart.

### 1.1 Relationship to source documents

| Source | What it specifies | Where this LLD refines it |
|---|---|---|
| ADR-0007 | The decision to extract `departments`/`plans`/`tender_acl` into three Wave services | §1, §19 |
| Document 1 (`01-hld-delta-decomposition.md`) §2.1 | "Stays in Core" vs. "moves out" table | §2 |
| Document 1 §4.1 | The `tenant_feature_flags` naming note — why the per-tenant override delta stays in Core | §2.2, §4.6, §16 (CAT-D2) |
| Document 1 §11 | Cross-service migration sequencing (Wave 1) | §19 |
| Document 1 §12 | Cross-service risk register | §6, §16 (CAT-D7) |
| `org_membership_lld_5.md` (`departments`/`plans` sections) | Original schema, invariants (D-1–D-11, OP-1–OP-7, PLAN-1–PLAN-6), cache design | §4, §5, §6, §10 |
| `iam-catalog-admin` (shipped implementation) | Ground truth once code exists — divergences tracked in the Revision History and §16 | Throughout, post-v1.1 |

### 1.2 Glossary

Terms specific to this service, plus the source-LLD terms most load-bearing for reading this document standalone:

- **Baseline** (`plans.feature_set` + named entitlement columns) — the per-tier entitlement defaults this service owns, shared by every tenant on a tier; merged at I-8 read time in Core with the per-tenant **override delta** (§4.6, PLAN-6).
- **Override delta** (`tenants.feature_flags`) — Core's own column, the per-tenant exception layered on top of the baseline; this service never reads or writes it (§2.2, §4.6).
- **`planDefaults(plan)`** — the function, owned by Core, that resolves a tenant's plan tier to its baseline entitlements; as of this extraction it reads this service's `plans` row (via `om:plans`) instead of a local `SELECT` or hardcoded map (§5.4, §19 step 2).
...
- **`platform_operator`** — the Keycloak role gating every write endpoint in this service (CAT-1, CAT-2, CAT-5); enforced at the service layer via `rc.Roles`, not a DB mechanism (§10).
- **Mesh-internal** — a route reachable only within the service mesh, mTLS-authenticated, with no external ingress and no JWT parsing (CAT-I1, CAT-I2, §10). This is an authorization/network property (the `iam-system` role check plus NetworkPolicy), **not** a distinct URL namespace — CAT-I1/CAT-I2 are served under the same `/api/v1` prefix as every public and operator route (CAT-D8, §16).
- **`om:departments` / `om:plans` / `gm:departments`** — the consumer-side caches Core and Group Mapping Service maintain of this service's catalogs, populated via CAT-I1/CAT-I2 on a miss (§6). `om:departments` and `gm:departments` are new as of this extraction; `om:plans` already existed in the source LLD under a different data source.
- **Stale-if-error** — the 24-hour-TTL fallback cache key each consumer maintains, served only when the primary key has expired **and** a live call to this service fails (§6, CAT-D4).
- **TTL self-heal** — this document's (and the source LLD's) standing cache-correctness posture: a missed invalidation is bounded by the key's own TTL and corrects itself on next expiry, rather than requiring an active push to be correct (§6, CACHE-3).
- **`record_version`** — the optimistic-locking column on both tables; a CAT-2/CAT-5 write must supply the current value or receive `409 optimistic_lock_conflict` (§5, §9.1, §17).
...
**What this service explicitly does *not* own** (see §2.2 and Document 1 §4.1): the per-tenant feature-flag override delta (`tenants.feature_flags`), which remains a column on Core's own `tenants` row, written by O-4, which stays in Core.

### 2.2 Out of scope (owned elsewhere)

| Concern | Owner | Why not here |
|---|---|---|
| Per-tenant feature-flag override delta (`tenants.feature_flags`) | Core Org & Membership | This service supplies only the *baseline* (`plans.feature_set` + the named entitlement columns); Core merges its own override delta at I-8 read time (§2.3, §4.6). This service has no concept of "tenant" at all — no `tenant_id` anywhere in its schema. |
| Pricing / billing | Billing Service | `plans` holds what a tier *grants*; what a tier *costs* is Billing's, per the same boundary the HLD already draws (HLD §7.3/§10.7). This service exposes no price, discount, or invoicing data. |
| Metering | Usage & Metering Service | Workflow-template and concurrent-tender limits are entitlement *ceilings*; consumption counters against those ceilings (and all LLM-token/API-request metering) belong to Metering, never here. |
| I-8 hot-path entitlement resolution | AuthZ Enrichment (via Core) | This service has no caller relationship with AuthZ Enrichment at all — direct or indirect. |
| Tenant-facing department/plan *management* | Core Org & Membership | Tenants read (`GET /api/v1/departments`); `GET /api/v1/tenants/:id/departments` stays in Core (§10). Only `platform_operator` writes here. |
| New entitlement dimensions independent of the HLD's feature matrix | HLD's plan-feature matrix (§6.6) | New named columns are added only when that matrix grows; everything else goes in `feature_set` — this service does not grow its own entitlement vocabulary unilaterally. |

### 2.3 The baseline vs. override split (design note)
...
`⊕` is the same flat, per-key, override-wins, scalar-only merge as before (PLAN-6(d)) — nested values are still rejected, still at write time (this service rejects a non-scalar `feature_set` value the same way Core rejects a non-scalar `feature_flags` override). **This service owns the left operand only.** Document 1 §4.1 records why the ADR's "`tenant_feature_flags`" shorthand does not imply a third table belongs here. §4.6 restates this boundary against the full data-ownership table; §16 (CAT-D2) records the decision not to relocate the override delta here.

---

## 3. Architecture and Package Layout

**Stack:** Go · Gin · pgx/v5 — identical technology baseline to every other IAM service (HLD §3.3), so the same `platform-gincommon`/`platform-pgcommon` shared libraries apply unchanged. Unlike every other IAM service, `platform-events` is **not** a dependency (§3.3.3, §7) — this service publishes and consumes nothing.

**Package layout** mirrors O&M's own layering (handler → service → repository), scaled down to reflect the much smaller domain: two aggregates (`Department`, `Plan`), no RLS layer, no tenant-context GUC bridge (there is no tenant context to bridge — this is the one IAM service in the whole stack that genuinely has none), and no outbox (§7). There is a single composition root (`cmd/catalog-admin-config/main.go`) — unlike `iam-user-profile`'s two (`cmd/server`, plus a `cmd/reconciler` CronJob for signature erasure), this service has no background reconciler process to give a second one to (§13).

```
iam-catalog-admin/
├── cmd/
│   └── catalog-admin-config/
│       ├── main.go                        # composition root — wires pool + cache + repos + services + handlers;
│       │                                  #   no GUCProvider (no RLS/tenant GUC, §10), no outbox/SQS drain
│       │                                  #   step in shutdown (§7)
│       ├── main_test.go                   # TestValidatePostgresConfig_*, TestValidateRequiredEnv_*
│       └── swagger_info.go                # swaggo @title/@version metadata
├── internal/
│   ├── core/
│   │   ├── domain/                        # entities, value objects, domain errors — no internal imports at all
│   │   │   ├── department.go              # Department (§4.2)
│   │   │   ├── plan.go                    # TenantPlan/BrandingLevel enums, Plan, PlanPatch (§4.1, §4.2)
│   │   │   └── errors.go                  # sentinel errors, DomainError, NewError, WithDetails (§17)
│   │   ├── port/                          # interfaces required by the core
│   │   │   ├── department_repository.go
│   │   │   ├── plan_repository.go
│   │   │   └── cache.go                   # Cache — advisory-only (CAT-FAIL-1, §9.4); nil is a valid config
│   │   └── service/                       # use cases
...
│   └── adapter/
│       ├── inbound/
│       │   └── http/                      # Gin handlers, DTOs, router, middleware
│       │       ├── department_handler.go  # CAT-1/2/3/6/7
│       │       ├── plan_handler.go        # CAT-4/5
│       │       ├── internal_handler.go    # CAT-I1/I2 — mesh-only bulk reads; same package as every other
│       │       │                          #   route, not a separate `internalapi` package (corrected v1.23)
│       │       ├── router.go              # single source of truth for routes/middleware (§8.4.0)
│       │       ├── middleware.go          # IdentityBridgeMiddleware, RequireOperatorRole, RequireSystemRole,
│       │       │                          #   RequireJSONContentType, NormalizeAuthErrors, HandleError (§10)
...
│           │   ├── migrate.go             # RunMigrations (embed.FS, bypasses PgBouncer, §4.4)
│           │   └── migrations/            # 000001_init_schema (single consolidated migration, §4.4)
│           ├── valkey/                    # cache impl (go-redis/v9 directly — no platform-* dependency)
│           │   └── cache.go               # DepartmentsKey/PlansKey constants, hit/miss metrics (§6)
│           └── metrics/                   # catalog_admin_* business metrics (cross-cutting — imported by
│               └── metrics.go             #   both the inbound HTTP adapter and the outbound valkey adapter)
├── pkg/
│   └── requestctx/
│       └── context.go                     # RequestContext{UserID, TenantID, Roles, ClientIP, UserAgent},
│                                          #   HasRole, IsOperator(), IsSystem()
├── docs/
│   ├── swagger/                           # OpenAPI 3 spec — GENERATED by `make swag` (swaggo), checked in;
│   │                                      #   CI `make swag-check` fails the build on any drift
│   ├── architecture/                      # README.md + mermaid/ (layer-model, write-flow, cache-strategy)
│   └── lld/iam-lld-catalog-admin-config-service.md   # this document
├── deploy/
│   ├── helm/                              # Chart.yaml, values.yaml, templates/ (2 replicas, no reconciler
│   │                                      #   CronJob — unlike iam-user-profile's signature-erasure one)
│   └── monitoring/                        # app-alerts.yml (PrometheusRule) + dashboards/ (one Grafana board)
├── test/                                  # unit/ (mocks, no Docker), postgres/ (testcontainers, -tags=integration),
│                                          #   e2e/ (full-stack HTTP), fixtures/, dbseed/ (pgcommon-backed seed SQL) —
│                                          #   no test/integration/ tier at all (§14): this service has no events/outbox
│                                          #   to cross-service-test
├── scripts/                               # merge_coverage.py, patch-swagger-extensions.py — local dev/CI tooling only
├── .github/                               # workflows/ (ci, validate-test, validate-quality, changelog-check,
│                                          #   release) + scripts/ (arch-lint, swag-check, coverage-gate, …)
├── ARCHITECTURE.md  CACHE_DESIGN.md  CHANGELOG.md  README.md
└── Dockerfile  docker-compose.yml  Makefile  go.mod  go.sum  .go-arch-lint.yml
```
...
`platform-events` (outbox + SNS publisher + SQS consumer) and `iam-keycloakclient` are both **not** dependencies of this service — see §3.3.3 and §7 for why. Gin middleware, logging, tracing, and the `/healthz` handler come from `platform-gincommon`; env-driven config resolution (`ConfigFromEnv`), the pgx pool, migrations, and structured pg-error matching (`IsUniqueViolation`/`IsCheckViolation`/`ConstraintName`/`IsConnectionException`/`IsInsufficientResources`/`IsPgError`) from `platform-pgcommon`, used **without** the `GUCProvider` option (§3.3.2) since there is no RLS/tenant-context GUC to inject for either `departments` or `plans` (§4.3, §10).
...
This section specifies exactly how the two platform libraries are integrated — which exported packages are used, where they are wired, and what is explicitly out of scope. Both are private Go modules under `github.com/BCBP-SOLUTIONS-FZC-LLC/`, consumed via the versions pinned in §3.1.
...
flowchart LR
    ...
    HTTP -->|IsConnectionException,<br/>IsInsufficientResources, IsPgError| PGC
    METRICS -->|MetricsRegisterer,<br/>MetricsConstLabels| GIN
    MAIN -->|ConfigFromEnv,<br/>NewPool, migrate.Runner,<br/>pgmetrics.InitWithRegisterer| PGC
    MAIN -->|"domain.Logger<br/>(Config.Logger, migrate.Runner.Logger)"| PGC
    MAIN -->|"Config.Tracer<br/>(NewOTelTracer → gincommon provider)"| PGC
    PG -->|RunInTx, IsUniqueViolation,<br/>IsCheckViolation, ConstraintName,<br/>IsConnectionException,<br/>IsInsufficientResources, IsPgError| PGC
```

#### 3.3.1 `platform-gincommon` — HTTP middleware, logging, tracing

This service is a **Gin HTTP service only**, with no gRPC surface. `gincommon.ObservabilityMiddlewares` provides panic recovery, request-ID, tracing, correlation headers, metrics, and structured logging; `gincommon.ProtectedMiddlewares` validates the gateway-injected `x-user-id`/`x-tenant-id` headers are present and well-formed — **no JWT parsing happens in this service or the shared library**, cryptographic verification is the gateway's job upstream. `gincommon.RequestContext(c)` is read by this service's own `IdentityBridgeMiddleware` (§10), which does only the identity-parsing half of O&M's `GUCBridgeMiddleware` — there is no RLS GUC to bridge. `NormalizeAuthErrors` rewrites bare 401 bodies from the shared auth middleware into this service's standard error envelope (§17), ported unchanged from `iam-org-membership`. `/healthz` is registered as `gincommon.HealthHandler()` directly (`internal/adapter/inbound/http/router.go`'s `registerInfraRoutes`) — not a local reimplementation of its `{"status":"ok"}` body; only `/readyz` (which checks Postgres + Valkey) has service-specific logic. As of `v1.3.0`, this service's own `catalog_admin_*` business metrics (`internal/adapter/outbound/metrics.Register`, §11.2) are registered via `gincommon.MetricsRegisterer()`/`MetricsConstLabels()` — the same registerer and `{service, version}` const labels gincommon's own `http_requests_total`/etc. carry — instead of `prometheus.MustRegister` against the bare default registry. `Register` is idempotent (`sync.Once`), needed because the e2e test suite constructs a fresh composition root per test in the same process. `cmd/catalog-admin-config/main.go` primes `ObservabilityMiddlewares` **before** any collector registration (matching `iam-org-membership`/`iam-realm-provisioner`); `NewRouter` re-applies the same slice onto the Gin engine (`metrics.Init` is `sync.Once`). `gincommon.InitTracingFromEnv()` is called unconditionally so in-process spans get valid trace IDs even when `OTEL_EXPORTER_OTLP_ENDPOINT` is unset — OTLP export is a no-op until a collector is configured.

#### 3.3.2 `platform-pgcommon` — config, pool, transactions, error helpers

Used **without** the `GUCProvider` option — the one deliberate difference from every RLS-scoped sibling service (§10). `pgcommon.ConfigFromEnv()` (`cmd/catalog-admin-config/main.go`) resolves `PG_*`/`DATABASE_URL` env vars into a `pgcommon.Config`, returning `[]ConfigWarning` for anything defaulted or questionable instead of silently discarding a parse error — this replaced a hand-rolled env-parsing block that had exactly that bug (`strconv.Atoi` on `PG_MAX_CONNS` with its error discarded, silently defaulting to 0 on a malformed value). Warnings are logged unconditionally; `validatePostgresConfig` then escalates specific insecure-config warnings (`PG_SSLMODE`/`DATABASE_URL`) to a startup panic, but **only** in `production`/`staging`, preserving this service's existing fail-fast posture. `DSNFromEnv`/`ApplyStatementTimeout`/`MigrationDSNFromEnv` (`internal/adapter/outbound/postgres/db.go`) remain this service's own thin helpers layered on top of the resolved DSN — `PG_STATEMENT_TIMEOUT` and PgBouncer-bypass routing are not `pgcommon` concerns. `DSNFromEnv` (added v1.28) is the single call site `main.go` uses to resolve the app DSN: it skips the `ApplyStatementTimeout` append when `DATABASE_URL` is set, since `pgcommon.ConfigFromEnv` returns that value verbatim and it may carry no `?` query string for `ApplyStatementTimeout` to safely append onto — the same guard `iam-org-membership`/`iam-realm-provisioner`'s identically-named helper applies. `ApplyStatementTimeout` is a no-op on an empty DSN and on a DSN that already contains `statement_timeout` (v1.30, matching those siblings). `MigrationDSNFromEnv()` takes no `appDSN` argument (v1.30): if `MIGRATION_DATABASE_URL` is set it applies `ApplyStatementTimeout` to that URL; otherwise it falls back to `DSNFromEnv()`. `pgcommon.NewPool` provides pooling, slow-query logging, and OTel query tracing, all of which are RLS-independent and used unchanged. `pgCfg.Tracer` is `pgadapter.NewOTelTracer` (v1.29) — a thin adapter onto gincommon's global TracerProvider, so every query emits a `db.query` span on the same OTLP pipeline as HTTP spans, matching `iam-org-membership`/`iam-realm-provisioner`. `pgmetrics.InitWithRegisterer` shares `gincommon.MetricsRegisterer()` so a single `/metrics` scrape serves HTTP + `catalog_admin_*` + `pgcommon_*` collectors together. `pgcommon.RunInTx` wraps every repository call in its own single-statement transaction (`withPool` in `db.go`) — there is no `TxRunner`/multi-statement seam here, because every write in this service's surface is single-row, single-table (CAT-FAIL-3, §9.4). Repository and service code matches Postgres errors structurally via `pgcommon.IsUniqueViolation`/`IsCheckViolation`/`ConstraintName` (`department_repository.go`'s `Insert`, `department_service.go`'s `Patch`) rather than a hand-rolled `pgconn.PgError` type-switch or message substring search — this replaced a local `isCheckViolation`/`contains`/`indexOf` helper set that duplicated exactly what `pgcommon` already exposes. As of `v1.3.0` (this service's v1.30), `wrapConnErr` additionally remaps SQLSTATE class `08`/`53` via `IsConnectionException`/`IsInsufficientResources` and class `57`/`58` via the `(SQLSTATE 57…)`/`(SQLSTATE 58…)` text in `Error()` (`isOperatorOrSystemErrorSQLState`) to `domain.ErrDBUnavailable`; other PgErrors pass through `IsPgError` so the service layer can still classify them. Inbound `HandleError` uses the same three helpers as a leaked-PgError fallback and **does not import `pgconn`** — matching `iam-org-membership`/`iam-realm-provisioner`. `migrate.Runner` applies `internal/adapter/outbound/postgres/migrations/*.sql` on startup; the migration DSN must bypass PgBouncer (session-scoped advisory lock) whenever `PG_BOUNCER_MODE=true` (§12). As of `v1.2.0`, `pgcommon.Config.Logger` and `migrate.Runner.Logger` are typed against a public `pkg/domain.Logger` interface (previously the unexported `port.Logger`, structurally impossible to implement from outside the module) — wired via `internal/adapter/outbound/postgres.NewDomainLogger`, a small adapter wrapping this service's own structured logger. Set on both `pgCfg.Logger` (`cmd/catalog-admin-config/main.go`, before `NewPool`) and `RunMigrations`'s logger parameter, so the slow-query tracer's WARN-level logs and each applied migration step now flow through the same structured logger as everything else, rather than being silently dropped (both fields are optional and no-op when nil, which is what every test call site passes).

#### 3.3.3 `platform-events` — not used

**Deliberately absent.** No outbox table, no outbox-runner goroutine, no SNS publisher, no SQS consumer, no `processed_events` dedup ledger, no AsyncAPI spec, and no `platform-events` entry in `go.mod` (§7). A write becomes visible to consumers purely through the `cat:*`/`om:*`/`gm:*` cache-TTL mechanism (§6) — there is no faster, event-driven path, by design, and therefore nothing for this library to do here.
...
No Postgres extensions are required beyond `pgcrypto`'s `gen_random_uuid()` (or the native `gen_random_uuid()` available on the platform's RDS Postgres version), used for `departments.id`. Two ENUM types are defined in this service's own schema — both moved here from Core at extraction time, since a Postgres ENUM type cannot be shared across databases (§4.6):

- **`tenant_plan`** — `'starter' | 'pro' | 'enterprise'`. `plans.code` (§4.2) is keyed directly off this type; there is no separate surrogate ID. Core's `tenants.plan` column keeps its own **local copy** of this ENUM (§4.6) — the two are kept in lockstep by convention (an operator-driven ENUM migration on both sides, PLAN-4), not by a shared type or a cross-database constraint.
- **`branding_level`** — `'none' | 'logo'`. `plans.custom_branding`'s type. No other table references it.
...
No column changes from the source LLD. This table's entire governance model, lifecycle state machine, and eleven invariants (D-1 through D-11 in the O&M LLD) transfer to this service **unchanged** — they described a global reference entity independent of which process serves it. The only substantive change is D-1's "never tenant-owned" now additionally reads "never Core-owned."

The Operator invariants that govern *who* may write this table and how retirement/immutability are enforced (OP-1 through OP-7 in the O&M LLD) transfer unchanged for the same reason: they describe a role-check (`platform_operator` via `rc.Roles`) and a lifecycle rule (hard-delete block, `is_system` immutability, non-cascading retirement, OP-5's "existing rows are untouched"), none of which depends on which process performs the check. §10 and §5 of this document restate the parts that are load-bearing for the endpoint catalog and security model; nothing in OP-1 through OP-7 is superseded or narrowed by this extraction.
...
Per the O&M LLD's own recommendation, the shipped schema uses **nullable limit columns with `NULL = unlimited`** rather than a `-1` sentinel (the LLD's §4.2 note flags this as the recommended final choice) — `workflow_template_limit`/`tender_limit` are `NULL` for Enterprise's unlimited tiers, `>= 0` otherwise (**CAT-D6**, §16).

All six `plans` invariants from the O&M LLD (PLAN-1 through PLAN-6, including PLAN-6's baseline-⊕-override merge rule) and the plan-specific Operator invariants (OP-6, OP-7) transfer to this service unchanged, for the same reason as the department invariants above: they describe a data shape, a PATCH-only write contract, and a read-time merge rule, none of which depends on which process enforces them. §4.6 below restates PLAN-6 in full because this service owns one side of that merge; PLAN-1 through PLAN-5 are otherwise unchanged from the source LLD and are not repeated here.

### 4.3 Row-Level Security

**None — by design, not by omission.** Neither `departments` nor `plans` carries a `tenant_id`; there is no RLS policy on either, no `app.tenant_id` GUC to inject, and no `GUCProvider` passed to `pgcommon.NewPool` (§3.3.2). This is the one IAM service in the stack with no RLS layer at all, because both tables are global reference data, not tenant-scoped rows (§2, §10.1). Where the source `org_membership_lld_5.md`'s own §4.3 "Tables without RLS" list already named `departments`/`plans`, this document simply confirms that treatment continues unchanged post-extraction.

### 4.4 Migrations

Applied on startup by `platform-pgcommon`'s `migrate.Runner` against the direct (non-PgBouncer) DSN (§3.3.2, §12) from `internal/adapter/outbound/postgres/migrations/`. As of v1.25, there is a single consolidated migration:

| Migration | Purpose |
|---|---|
| `000001_init_schema` | Everything: both ENUM types (§4.1), the `departments`/`plans` tables (§4.2) and their seed data, all four lifecycle triggers (§4.5, including `prevent_system_department_name_change`'s structured `ERRCODE = 'check_violation'`/synthetic `CONSTRAINT` — previously added by a later migration, now present from the start), the two indexes, and the `catalog_admin_app` role/grants (§10.3). Confirmed byte-identical against the shipped file as of the v1.25 audit. |

This was previously split across four files (`000001_init_schema`, `000002_triggers`, `000003_roles_grants`, `000004_department_name_immutable_errcode`), consolidated at v1.25 into the single file above — safe because this service has never been deployed to any environment, so no environment holds the earlier multi-file history that a forward-only migration convention would otherwise require preserving. A short-lived `000004_audit_log`/`000005_audit_log_grants` pair existed between v1.9 and v1.10 and was fully reverted (§16, CAT-D10) before the consolidation — not part of the current migration set, mentioned here only so a reader cross-checking the revision history against a live migrations directory does not conclude one is missing.

### 4.5 Triggers — `touch_row` and lifecycle guards

Four triggers, ported verbatim from the O&M LLD, live on `departments`; `plans` carries the equivalent `touch_row` behavior for its own `record_version`/`updated_at` bump on every `UPDATE` (CAT-5).

| Trigger | Table | Fires on | Effect |
|---|---|---|---|
| `touch_row` | `departments`, `plans` | `BEFORE UPDATE` | Bumps `record_version` and sets `updated_at = now()` — the mechanism behind every `409 optimistic_lock_conflict` probe (§9.1). |
| `prevent_department_delete` | `departments` | `BEFORE DELETE` | Hard-blocks any `DELETE` unconditionally (D-4) — belt-and-suspenders alongside the withheld `DELETE` grant (§10.3) and CAT-3's `405` at the API layer. |
| `prevent_department_code_change` | `departments` | `BEFORE UPDATE` | Rejects any change to `code` once set (D-2/D-10) — the DB-level backstop for the `422 field_immutable` check `department_handler.go`'s `Patch` already performs on the raw request body. |
| `prevent_system_department_name_change` | `departments` | `BEFORE UPDATE` | Rejects a `name` change while `is_system = true` (D-11), raising `422 system_name_immutable` via `pgcommon.ConstraintName`, per the structured SQLSTATE/constraint metadata the trigger raises with (§4.4). |

`chk_system_department_active` (§4.2's inline CHECK, not a trigger) is what actually raises `422 system_department_cannot_be_retired` when a `PATCH` attempts to set `is_active=false` on a system department — listed here for completeness since CAT-2's sequence diagram (§8.4.2) shows both the trigger-raised and CHECK-raised paths together.

### 4.6 Ownership boundary: what this table is *not*

`plans.feature_set` is the **baseline** — shared by every tenant on a tier. `tenants.feature_flags` (Core's column) is the **per-tenant override delta**. This service never reads or writes `tenants.feature_flags`, has no visibility into any tenant's override state, and computes no "effective" value itself — that merge happens exclusively in Core, at I-8 read time, per PLAN-6's unchanged rule (restated in full at §2.3):
...
erDiagram
    ...
    PLANS {
        ...
        jsonb feature_set "baseline only — PLAN-6 merge happens in Core (§4.6)"
        bigint record_version "optimistic lock; CHECK > 0"
        timestamptz created_at
        timestamptz updated_at
    }
...
- **API version** in the path: every route is under `/api/v1`, including the mesh-only internal routes (**CAT-D8**, §16) — there is no separate prefix-free `/internal/*` namespace.
- All routes sit behind the middleware chain detailed in §8.4.0: `ProtectedMiddlewares` (gateway identity headers) → `IdentityBridgeMiddleware` (role parse) → a route-group role gate (`RequireOperatorRole`/`RequireSystemRole`, where applicable) → an in-handler defense-in-depth re-check (`requireOperator`/`requireSystem`) before any business logic runs.
- The service **trusts the gateway-injected headers** `x-user-id`, `x-tenant-id` (mTLS guarantees they came from the gateway, matching every sibling IAM service's contract) — there is no JWT parsing in this service; `IdentityBridgeMiddleware` only validates header presence/shape and parses `x-tenant-roles` into `rc.Roles`. `x-tenant-id` is required and logged on every request even though this service never consults the parsed value for anything (§10.1, **CAT-D12**, §16).
- Content type `application/json`, enforced by `RequireJSONContentType` ahead of every write route (`415 unsupported_media_type` otherwise, §17). Timestamps are RFC 3339 UTC. Request bodies are capped at 1 MB, checked twice (`Content-Length` pre-check plus `http.MaxBytesReader` for chunked requests, `413 request_entity_too_large` otherwise, §17).
- Errors use the platform-wide `gincommon.ErrorResponse` shape (§17).
- **Mutation and single-resource responses include `record_version`** so clients can round-trip the optimistic-concurrency token (§9.1) — every `departments`/`plans` row carries it, and CAT-2/CAT-5's responses (and `409 optimistic_lock_conflict`'s body) surface the current value directly.
- Three route classes:
  - **Public routes** (`GET /api/v1/departments[/:id]`, CAT-6/CAT-7) — any authenticated caller, no role beyond a valid identity.
  - **Operator routes** (`/api/v1/operator/...`, CAT-1–CAT-5) — `platform_operator`-gated, `RequireOperatorRole` at the route-group level plus an in-handler `requireOperator` re-check.
  - **Internal routes** (`/api/v1/internal/...`, CAT-I1/CAT-I2) — mesh-only, `iam-system`-role-gated (`RequireSystemRole` plus an in-handler `requireSystem` re-check) and additionally restricted by NetworkPolicy; no external ingress terminates here (§10.4).

### 5.2 Authorization rules per route

**Public routes (CAT-6, CAT-7).** Any authenticated caller may read the full department catalog or a single department by ID — there is no ownership or tenant-scoping concept to check (§2), so the only gate is a valid identity header pair.

**Operator routes (CAT-1–CAT-5).** Every write, and every operator-facing read, requires the `platform_operator` role, parsed from the gateway-injected role header via `rc.Roles`. This is a **service-layer check, not a DB mechanism** (§10.3) — the DB grants `SELECT` broadly and restricts `INSERT`/`UPDATE` to the single `catalog_admin_app` role used for both migrations and app traffic; the role gate that actually decides *who* may call the write endpoints lives entirely in `RequireOperatorRole` plus each handler's own `requireOperator` re-check, mirroring OP-1/OP-6/OP-7's enforcement pattern from the source LLD unchanged.

**Internal routes (`/api/v1/internal/*`, CAT-I1, CAT-I2).** Not user-facing. Reachable only from in-mesh service callers (Core Org & Membership, Group Mapping Service) behind a NetworkPolicy allow-list; authorized by the `iam-system` role via `RequireSystemRole` plus an in-handler `requireSystem` re-check — never by path shape (**CAT-D8**, §16). No JWT parsing, no tenant context to check (§10.4).

### 5.3 Endpoint catalogue

| ID (new) | Old ID | Method & path | Purpose | AuthZ | Cached |
|---|---|---|---|---|---|
| CAT-1 | O-1 | `POST /api/v1/operator/departments` | Create a department | `platform_operator` | invalidates `cat:departments` |
| CAT-2 | O-2 | `PATCH /api/v1/operator/departments/:id` | Update `name`/`is_active`, optimistic-locked. Unchanged behaviour, incl. `400 no_mutable_field` (neither present), `404`, `422 system_department_cannot_be_retired`, `422 system_name_immutable`, `422 field_immutable`, `409 optimistic_lock_conflict` | `platform_operator` | invalidates `cat:departments` |
| CAT-3 | O-3 | `DELETE /api/v1/operator/departments/:id` | Still hard-blocked, `405 Method Not Allowed` — the `platform_operator` gate (`RequireOperatorRole` + an in-handler `requireOperator` re-check, same defense-in-depth pattern as CAT-1/2/4/5) runs *before* the 405, so an unauthenticated/unauthorized caller gets `401`/`403`, never reaching the block (corrected v1.23; this row previously showed no auth gate) | `platform_operator` | — |
| CAT-4 | O-5 | `GET /api/v1/operator/plans[/:code]` | Read the plan catalog or a single tier (operator-facing) | `platform_operator` | yes (`cat:plans`) |
| CAT-5 | O-6 | `PATCH /api/v1/operator/plans/:code` | Update a plan tier, PATCH-only, `record_version`-optimistic-locked | `platform_operator` | invalidates `cat:plans` |
| CAT-6 | *(undocumented)* | `GET /api/v1/departments` | Global catalog listing, read-only, `active_only` filter applied in-memory after the cache read. Newly catalogued per ADR-0007 Action Item 1 | any authenticated caller | yes (`cat:departments`) |
| CAT-7 | *(undocumented)* | `GET /api/v1/departments/:id` | Single department read. Newly catalogued | any authenticated caller | yes (`cat:departments`) |
| CAT-I1 | *(new)* | `GET /api/v1/internal/departments` | Bulk read, full catalog, for cache population (§5.4, §8.4.8) | mesh-only (`iam-system`) | yes (`cat:departments`) |
| CAT-I2 | *(new)* | `GET /api/v1/internal/plans` | Bulk read, all three tiers, for cache population (§5.4, §8.4.9) | mesh-only (`iam-system`) | yes (`cat:plans`) |
...
#### CAT-1 / CAT-2 — department write validation

CAT-1 (`POST`) requires non-blank `code` and `name` (`400 validation_error` otherwise); `code` must be unique across the table (`409 duplicate_code` on `uq_departments_code`, D-10). CAT-2 (`PATCH`) accepts only `name` and/or `is_active` in the body — `code`/`is_system` are pre-read from the raw JSON body and rejected with `422 field_immutable` before DTO binding would otherwise silently drop them (§8.4.2); a body with neither mutable field set returns `400 no_mutable_field`; every write carries `record_version` for the optimistic-lock check (§9.1).

#### CAT-5 — `feature_set` and limit validation

CAT-5's `PATCH` body may set any of `display_name`, `workflow_template_limit`, `tender_limit`, `trial_duration_days`, `sso_enabled`, `custom_branding`, or `feature_set`. `workflow_template_limit`/`tender_limit` use a **double-pointer** JSON decode (`parseLimit`, via `json.RawMessage`) to distinguish three states the wire format cannot otherwise express: the field absent (no `SET` clause at all), explicit `null` (`SET ... = NULL`, meaning unlimited, **CAT-D6**), and an explicit integer (`SET ... = <value>`, capped, `400 validation_error` if negative) — this is why `domain.PlanPatch` carries these two fields as `**int` rather than `*int` (§4.2). `feature_set` values must be scalar (string/bool/number/nil); a nested object/array returns `422 invalid_feature_value` naming the offending `key` (PLAN-6(d), §4.6). `custom_branding` is validated against the `branding_level` ENUM values. Every write carries `record_version` (§9.1).

#### CAT-I1 — `GET /api/v1/internal/departments`
...
**Why acceptable:** these call sites already tolerate multi-hundred-millisecond admin-write latency today; a 30 ms internal hop is immaterial. See §9.3 for the failure-mode table.

#### CAT-I2 — `GET /api/v1/internal/plans`

**Purpose:** bulk read of the three-tier entitlement catalog, replacing Core's local `SELECT * FROM plans` as the source for `om:plans`, which feeds `planDefaults(plan)` in I-8's effective-features merge (§4.6).
...
| `201` | CAT-1 create success |
| `400` | Malformed request — missing/blank required field, invalid limit value, too-large negative, bad UUID path param (`invalid_uuid`) |
| `401` | Missing/invalid identity headers (`missing_identity_headers`) |
| `403` | Caller authenticated but lacks the required role (`insufficient_role`) |
| `404` | Department/plan not found within scope (`department_not_found`/`plan_not_found`) |
| `405` | CAT-3's hard-blocked `DELETE`, or any route hit with a disallowed verb (`method_not_allowed`) |
| `409` | Optimistic-lock conflict (CAT-2/CAT-5) or a unique-constraint conflict (CAT-1, `duplicate_code`) |
| `413` | Request body exceeds the 1 MB cap (`request_entity_too_large`) |
| `415` | `Content-Type` is not `application/json` on a write route (`unsupported_media_type`) |
| `422` | Semantically invalid — immutable field, system-department lifecycle rule, non-scalar `feature_set` value |
| `503` | DB/cache dependency unavailable (`db_unavailable`/`dependency_unavailable`) |

The full machine-readable error taxonomy, including every code's HTTP status and any extra response field, is in §17.
...
### 6.2 Propagation delay and reconciliation
...
**Reconciling the two propagation-delay figures in the source LLD.** PLAN-3 states "Operators should expect up to ~5 min for a plan-catalog change to be universally visible, not immediate" — this is the *expected-case* number: a randomly-timed edit sees, on average, half of the `om:plans` 600 s TTL elapse before the next read re-populates the cache (600 s / 2 ≈ 5 min). The **worst case** implied by the same TTL is the full 600 s (10 min), which is the figure this document uses when stating a hard bound (§16, CAT-D3) rather than an expectation — the two numbers describe the same mechanism from two different vantage points, not a discrepancy. Separately, and **not fully carried over from the source LLD**, PLAN-3 also notes that evicting `om:plans` does **not** actively evict Core's per-user `om:memberships:{tenant}:{user}` projection: an already-cached user's entitlements only pick up a `plans` edit at their *next* I-8 back-fill, gated by that key's own 300 s TTL. This second hop is entirely Core's concern — this service has no visibility into `om:memberships` — but it is the reason the source LLD's "~5 min" figure is best read as an end-to-end, two-cache estimate rather than just `om:plans`'s own TTL/2; anyone setting operator expectations off this document should account for that additional hop, not just the `om:plans`/`om:departments` layer described here.

The same TTL-only reasoning now additionally covers cross-service propagation: **a webhook-based active-invalidation option was considered and deferred** (tracked as a decision-register item, §16) — it would shrink the staleness window from the 600 s TTL to near-zero, at the cost of Catalog Service needing to know its consumers' identities and cache-invalidation endpoints, which is exactly the kind of coupling this decomposition is trying to remove. TTL-only is the shipped design; a push-based option remains available if operational experience shows the window is a real problem.

### 6.3 Stale-if-error fallback

**Cold-cache-plus-dependency-down mitigation (addresses Document 1 §12's risk item):** each consuming service (Core, Group Mapping) additionally maintains a longer-TTL "stale-if-error" fallback key (`om:departments:stale`, `om:plans:stale`, `gm:departments:stale`; 24 h TTL, refreshed opportunistically on every successful CAT-I1/CAT-I2 call) that is served **only** when the primary key has expired **and** the live call to Catalog Service fails. This converts "Catalog Service is down and my cache just expired" from a hard failure on the consuming service's write path into a serve-stale-with-a-logged-warning outcome, bounded at 24 hours — well inside any reasonable Catalog Service MTTR (**CAT-D4**, §16).

### 6.4 Department retirement staleness and cache failure mode

**The same TTL-bounded staleness has a sharper implication for `departments` than for `plans`, worth calling out separately (CAT-D7, §16): it now governs a correctness-adjacent access gate, not just an entitlement value.** Core's D-5/TD-1 rule — a retired department (`is_active = false`) may not receive new `tenant_departments` activations or `dept_memberships` — was previously enforced against a live row (checked in the same request as the FK-guaranteed existence check); it is now enforced against the cached `om:departments` snapshot (§4.7, §19 step 4). Practically: an operator who retires a department in Catalog Service should expect Core (and Group Mapping, via `gm:departments`) to keep accepting new activations/mappings against it for up to the consuming cache's TTL (600 s, or up to 24 h in the stale-if-error fallback case above) before the retirement is actually honored tenant-side. This is a materially different risk profile than a stale `plans` read, which self-corrects at the user's next lookup with no lasting side effect — a department wrongly accepted as active during the staleness window produces real `tenant_departments`/`dept_memberships`/`group_dept_mappings` rows that, per D-6/OP-5, are **not** automatically cleaned up once the retirement does propagate. An operator retiring a department for a hard business reason (not just tidying an unused entry) should treat this window as an operational constraint to plan around, not a cosmetic delay. This flow is worked through end-to-end at §8.1.

**Cache failure mode within this service itself.** `cat:departments`/`cat:plans` is advisory (**CAT-FAIL-1**, §9.4); a miss or a Valkey outage falls through to Postgres, never to a wrong answer — see §8.3 for the read-through mechanism and §9.3 for the full failure matrix.

---

## 7. Event Architecture

This service publishes and consumes no events at all — a structural simplification versus every other service in the IAM stack (all of which carry the transactional-outbox pattern, HLD §4.1). The subsections below state each of the platform's standard event-architecture checklist items explicitly as "N/A, and here is why," in the same order every sibling LLD covers them, rather than leaving any of them as an unexplained gap.

### 7.1 Inbound — SQS consumers

**None.** This service subscribes to no SNS topic and runs no SQS consumer. There is no external event this service needs to react to: it has no `tenant_id` column anywhere in its schema (§2.2), so it has nothing to key a tenant-lifecycle cascade off in the first place — unlike the Group Mapping Service (Document 3 §10.1), which does hold tenant-scoped rows and therefore does subscribe to the tenant-offboarding relay. Confirmed this is a deliberate absence, not an oversight: every table this service owns is global, and a global catalog row is never invalidated by a tenant's lifecycle.
...
### 7.2 Outbound — SNS topics and event types
...
### 7.4 AsyncAPI / schema governance participation

This service ships **no `api/asyncapi.yaml`** and is **not registered** with the `platform-schemagov` pipeline (§19) — there is no event type for `schema-gov extract`/`validate`/`register` to act on. This is an explicit, documented exemption rather than a gap: a future reviewer checking why this service has no CI job wired to `schema-registry.yml` should find this paragraph, not an unexplained absence. If a genuine event need ever arises here (for example, a future `DepartmentCatalogChanged` notification to downstream caches, in place of today's TTL-only propagation, per §16 CAT-D3), it re-enters the normal `schema-gov` pipeline at that time, exactly as every other IAM event does.

#### 7.4.1 AWS Glue Schema Registry

**N/A — no registry entry exists for this service, for the same reason as §7.4.** Services that publish events (e.g. User Profile, `iam-lld-user-profile.md` §7.3.1) register each event type's JSON Schema as a Glue schema version and encode/decode payloads against it on publish/consume. This service has no event type to register a schema for, no Glue registry name configured in its environment (§12), and no `github.com/aws/aws-sdk-go-v2/service/glue`/`.../schemas` dependency in `go.mod`. Stated explicitly here, alongside §7.4, so a reviewer checking this service against the platform's full event-architecture checklist (AsyncAPI, schema governance, Glue Schema Registry) finds three consistent "N/A, and here is why" statements rather than two explained absences and one unexplained gap.
...
**This service does not build a local stand-in for that mechanism (CAT-D10, §16).** CAT-1/CAT-2/CAT-5 writes today produce no durable, queryable audit record beyond structured request logging (`gincommon.ObservabilityMiddlewares`) — the same posture O&M's own operator writes have, since O&M's LLD also never specifies a durable store for the "audit entry" it references. Once the Audit Log Service's direct-write contract is actually specified, this service should call it the same way — that is the intended fix, tracked as an open item (§16), not resolved by this document inventing a one-off table against a contract that doesn't yet exist.

### 7.7 Event architecture invariants
...
| CAT-EVT-3 | This service is **not** registered with `platform-schemagov` and ships no `asyncapi.yaml` — an explicit, documented exemption (§7.4), not an omission. |
| CAT-EVT-4 | Write visibility to consumers is governed entirely by cache TTL (§6), not by event propagation — there is no sub-TTL delivery guarantee and none is promised. |
...
### 8.1 Department retirement and downstream propagation

An operator sets `is_active=false` on a department via CAT-2 (§8.4.2). The write commits and `cat:departments` is invalidated immediately (§6.1) — this service's own `GET /api/v1/departments[/:id]` reflects the retirement on the very next read. Core and Group Mapping, however, continue serving their own `om:departments`/`gm:departments` cached copies until that key's TTL expires (600 s), or — if this service is unavailable when they next miss — until the 24 h stale-if-error ceiling (§6.3). During that window, Core's D-5/TD-1 rule ("no new activation against a retired department") is being checked against a snapshot that still shows the department active, so new `tenant_departments`/`dept_memberships` rows can still be created against it (§6.4, **CAT-D7**, §16). Those rows are not automatically cleaned up once the retirement does propagate (D-6/OP-5) — an operator retiring a department for a hard business reason should plan around this window (§20.3), not treat it as instantaneous.

### 8.2 Plan catalog edit propagation

An operator PATCHes a plan tier via CAT-5 (§8.4.5) — say, raising Starter's `workflow_template_limit`. The write commits, `cat:plans` invalidates immediately, and this service's own `GET /api/v1/operator/plans[/:code]` reflects the change at once. Core's `om:plans` cache, however, serves the pre-edit value until its own TTL expires — an expected ~5 minute (600 s / 2) delay on average, up to a 10-minute worst case (§6.2). A tenant on that plan whose entitlements are already cached in Core's per-user `om:memberships:{tenant}:{user}` projection sees the change only at their *next* I-8 back-fill (that key's own 300 s TTL) — a second hop entirely inside Core's own cache layers, not visible to or controllable by this service. Unlike a department retirement (§8.1), a stale plan read has no lasting side effect: the next lookup after either TTL expires simply reflects the new value, with nothing left over to clean up.

### 8.3 Cache mechanism — read-through with stale-if-error fallback

The mechanism referenced by every read endpoint below (CAT-4, CAT-6, CAT-7, CAT-I1, CAT-I2) and by the two flows above:
...
flowchart TD
    ...
    subgraph consumer["Consumer side (e.g. iam-org-membership) — §6"]
...
### 8.4 API sequence diagrams (per §5.3 endpoint)

A sequence diagram per §5.3 endpoint. To avoid repetition, every request first passes the shared **Request Preamble** (§8.4.0); the per-endpoint diagrams start *after* it, at this service's handler layer, assuming the identity headers and role have already been parsed. Endpoints CAT-1–CAT-7 are reached by ordinary authenticated/operator callers through the gateway; CAT-I1/CAT-I2 are mesh-internal service-to-service calls and are drawn starting directly at the `RequireSystemRole` check instead, since they carry no end-user identity to bridge.

#### 8.4.0 Request preamble (shared by every endpoint)
...
sequenceDiagram
    ...
    participant MW as ProtectedMiddlewares /<br/>IdentityBridgeMiddleware
    participant RG as Route-group role gate<br/>(RequireOperatorRole /<br/>RequireSystemRole, where applicable)
    participant H as Service handler

    Caller->>GW: HTTP request
    GW->>GW: mTLS-authenticate caller, inject x-user-id / x-tenant-id / x-tenant-roles
    GW->>MW: forward request + identity headers
    MW->>MW: validate headers present and well-formed
    alt headers missing or malformed
        MW-->>Caller: 401 missing_identity_headers
    end
    MW->>MW: parse rc.Roles from x-tenant-roles
    MW->>RG: c *gin.Context
    alt route requires a role and caller lacks it
        RG-->>Caller: 403 insufficient_role
    end
    RG->>H: c *gin.Context
    Note over H: every handler re-checks its own role requirement<br/>in-handler (requireOperator/requireSystem) as defense in depth,<br/>even though RG already gated it (§10.3, §10.4)
```

#### 8.4.1 CAT-1 — `POST /api/v1/operator/departments`
...
sequenceDiagram
    ...
    Op->>MW: POST /api/v1/operator/departments
    ...
    H->>H: validate code/name non-blank (400 validation_error)
    H->>S: Create(ctx, code, name)
    S->>R: Insert(ctx, code, name)
    R->>DB: INSERT INTO departments (...) VALUES (...)
    DB-->>DB: uq_departments_code may reject (409 duplicate_code, D-10)
    DB-->>R: new row, or unique-violation error
    R-->>S: *domain.Department, error
    S->>C: DEL cat:departments (invalidate)
    S-->>H: *domain.Department, error
    H-->>Op: 201 + body, or mapped error status
```

#### 8.4.2 CAT-2 — `PATCH /api/v1/operator/departments/:id` (write, optimistic-locked, dual 422 outcomes)

Two representative flows — a write (CAT-2) and a cached read spanning this service and a consumer (CAT-6/CAT-I1, §8.3) — cover every mechanism the remaining endpoints reuse (optimistic locking, defense-in-depth role re-checks, read-through/stale-if-error caching). This diagram is reproduced from the shipped implementation's own `ARCHITECTURE.md`, not drafted ahead of the code, so it reflects what CI actually exercises rather than an aspirational flow.
...
sequenceDiagram
    ...
    DB-->>DB: touch_row bumps record_version, updated_at (§4.5)
    DB-->>DB: chk_system_department_active may reject (422 system_department_cannot_be_retired)
    DB-->>DB: prevent_system_department_name_change may reject (422 system_name_immutable, D-11)
...
#### 8.4.3 CAT-3 — `DELETE /api/v1/operator/departments/:id` (hard-blocked)
...
sequenceDiagram
    ...
    participant MW as Middleware chain
    participant H as DepartmentHandler
    Op->>MW: DELETE /api/v1/operator/departments/:id
    MW->>MW: ProtectedMiddlewares (identity headers)
    MW->>MW: IdentityBridgeMiddleware (parse role)
    MW->>MW: RequireOperatorRole (platform_operator?)
    MW->>H: c *gin.Context
    H->>H: requireOperator(c) — defense-in-depth re-check
    H-->>Op: 405 method_not_allowed (D-4 — no department is ever physically deletable)
    Note over H,Op: An unauthenticated/unauthorized caller never reaches this line —<br/>401/403 is returned by the role gate above first (corrected v1.23, §5.3)
```

#### 8.4.4 CAT-4 — `GET /api/v1/operator/plans[/:code]`
...
sequenceDiagram
    ...
    participant R as PlanRepository
    participant DB as Postgres
    participant C as Valkey

    Op->>MW: GET /api/v1/operator/plans[/:code]
    MW->>MW: ProtectedMiddlewares (identity headers)
    MW->>MW: IdentityBridgeMiddleware (parse role)
    MW->>MW: RequireOperatorRole (platform_operator?)
    MW->>H: c *gin.Context
    H->>H: requireOperator(c) — defense-in-depth re-check
    H->>S: List(ctx) / Get(ctx, code)
    S->>C: GET cat:plans
    alt cache hit
        C-->>S: cached catalog
    else cache miss
        S->>R: List(ctx) / Get(ctx, code)
        R->>DB: SELECT * FROM plans [WHERE code=$1]
        DB-->>R: rows, or 0 rows (404 plan_not_found for :code)
        R-->>S: []*domain.Plan / *domain.Plan
        S->>C: SET cat:plans (60s TTL)
    end
    S-->>H: plan(s)
    H-->>Op: 200 + body, or mapped error status
```

#### 8.4.5 CAT-5 — `PATCH /api/v1/operator/plans/:code`
...
sequenceDiagram
    ...
    Op->>MW: PATCH /api/v1/operator/plans/:code
    MW->>MW: ProtectedMiddlewares (identity headers)
    MW->>MW: IdentityBridgeMiddleware (parse role)
    MW->>MW: RequireOperatorRole (platform_operator?)
    MW->>H: c *gin.Context
    H->>H: requireOperator(c) — defense-in-depth re-check
    H->>H: parseLimit — double-pointer absent/null/value distinction (§5.4, CAT-D6)
    H->>H: validate feature_set values are scalar (422 invalid_feature_value + key)
    H->>H: validate custom_branding enum, non-negative limits (400 validation_error)
    H->>S: Patch(ctx, code, ..., expectedVersion)
    S->>R: Update(ctx, code, ..., expectedVersion)
    R->>DB: UPDATE plans SET ... WHERE code=$1 AND record_version=$2
    DB-->>DB: touch_row bumps record_version, updated_at (§4.5)
    DB-->>R: updated row, or 0 rows (404/409 probe)
    R-->>S: *domain.Plan, error
    S->>C: DEL cat:plans (invalidate)
    S-->>H: *domain.Plan, error
    H-->>Op: 200 + body, or mapped error status
```

#### 8.4.6 CAT-6 — `GET /api/v1/departments`
...
sequenceDiagram
    ...
    U->>MW: GET /api/v1/departments?active_only=
    MW->>MW: ProtectedMiddlewares (identity headers)
    MW->>MW: IdentityBridgeMiddleware (parse role — no role required beyond authenticated)
    MW->>H: c *gin.Context
    H->>S: List(ctx)
    Note over S: cat:departments read-through (§8.3) — the same cache entry CAT-I1 populates
    S-->>H: full catalog
    H->>H: apply active_only filter in-memory, after the cache read
    H-->>U: 200 + filtered body
```

#### 8.4.7 CAT-7 — `GET /api/v1/departments/:id`
...
sequenceDiagram
    ...
    U->>MW: GET /api/v1/departments/:id
    MW->>MW: ProtectedMiddlewares (identity headers)
    MW->>MW: IdentityBridgeMiddleware (parse role)
    MW->>H: c *gin.Context
    H->>H: parseUUIDParam(:id) — 400 invalid_uuid if malformed
    H->>S: List(ctx) — same cat:departments read-through as CAT-6 (§8.3)
    S-->>H: full catalog
    H->>H: find id in catalog — 404 department_not_found if absent
    H-->>U: 200 + single department body
```

#### 8.4.8 CAT-I1 — `GET /api/v1/internal/departments`
...
sequenceDiagram
    ...
    H->>S: List(ctx) — same cat:departments read-through as CAT-6/CAT-7 (§8.3)
    S-->>H: full catalog
    H-->>Core: 200 { departments[], as_of }
```

#### 8.4.9 CAT-I2 — `GET /api/v1/internal/plans`
...
sequenceDiagram
    ...
    Core->>MW: GET /api/v1/internal/plans (mTLS, mesh-internal)
    MW->>MW: ProtectedMiddlewares (identity headers)
    MW->>MW: IdentityBridgeMiddleware (parse role)
    MW->>MW: RequireSystemRole (iam-system?)
    MW->>H: c *gin.Context
    H->>H: requireSystem(c) — defense-in-depth re-check
    H->>S: List(ctx) — same cat:plans read-through as CAT-4 (§8.3)
    S-->>H: full catalog + per-item record_version
    H-->>Core: 200 { plans[], record_versions{} }
```
...
CAT-2 and CAT-5 use the monotonic **`record_version`** column (§4.2, bumped by each table's `touch_row` trigger, §4.5) as the version token — not `updated_at`, for the same reason every optimistic-lock design on this platform avoids timestamp equality under sub-millisecond concurrent writes. Both PATCH bodies carry `record_version` (the value the client last read); the write executes `UPDATE ... SET ... WHERE id/code=$1 AND record_version=$2` (§8.4.2, §8.4.5). Zero rows affected means either the row doesn't exist or a concurrent writer already won — the repository probes which case applies and the handler returns `404`/`409 optimistic_lock_conflict` accordingly. The `409` body carries the row's actual current `record_version` (§17) so a client can retry without a second round-trip. Given how rare writes are on this service (§2 — "writes happen only on operator action"), a sustained nonzero conflict rate is itself diagnostic of a caller retry-storm or a tooling bug rather than expected contention (§11.2's `catalog_admin_optimistic_lock_conflicts_total` metric and its alert exist for exactly this).

### 9.2 Transaction discipline

`pgcommon.RunInTx` wraps every repository call in its own single-statement transaction (`withPool`, `internal/adapter/outbound/postgres/db.go`, §3.3.2) — there is no `TxRunner`/multi-statement seam here, because every write in this service's surface is single-row, single-table (**CAT-FAIL-3**, below). Repository and service code matches Postgres errors structurally via `pgcommon.IsUniqueViolation`/`IsCheckViolation`/`ConstraintName` rather than message substring matching (§3.3.2).

### 9.3 Dependency failure matrix

| Scenario | Detection | Effect on this service | Effect on callers |
|---|---|---|---|
| Catalog Service DB down | Health check / `/readyz` fails | Service marks itself not-ready; writes rejected `503` | Core/Group Mapping serve from cache (TTL, then stale-if-error fallback, §6); no impact to I-8 |
...
| CAT-FAIL-4 | This service has no outbox, no SNS publisher, and no SQS consumer (§7) — the entire "outbox crash/redelivery/DLQ" failure class that applies elsewhere in the IAM stack does not exist here. |

---

## 10. Security

### 10.1 Tenant isolation — none required

**Global tables, no RLS.** Neither `departments` nor `plans` carries a `tenant_id`; there is no row-level security policy on either, matching their treatment in the O&M LLD today (§4.3's "Tables without RLS" list already named both) and restated at §4.3.

### 10.2 App role and CI enforcement

**`org_membership_app`-equivalent role.** This service's DB role (`catalog_admin_app`) has no `BYPASSRLS` grant (there is nothing to bypass), and CI verifies this directly — `TestRoles_AppRoleHasNoBYPASSRLS` (`test/e2e/roles_test.go`), mirroring O&M's own `TestRLS_Case1b_AppRoleHasNoBYPASSRLS`, queries `pg_roles.rolbypassrls` against a real Postgres and fails the build if it's ever true. This assertion previously existed only as a claim in this document, with no actual test or CI job behind it — added when a reconciliation pass found the gap.

### 10.3 Write authorization

**Write authorization is a service-layer role check, not a DB mechanism.** Every write endpoint (CAT-1, CAT-2, CAT-5) re-checks `rc.Roles` for `platform_operator` in the handler, exactly mirroring OP-1/OP-6/OP-7's existing enforcement pattern (§5.2, §8.4). The DB grants `SELECT` broadly (mesh-readable) and restricts `INSERT`/`UPDATE` to the same single `catalog_admin_app` role used for both migrations and app traffic — deliberately not split into a separate migrator/app-role pair (`000001_init_schema.up.sql`'s own comment: "no migrator/admin_readonly split is needed — no cross-tenant data exists to read around"). No role is granted `DELETE` at all; hard-delete is triggered to fail regardless (OP-3/D-4, §4.5), and the grant is withheld too, in defense in depth.

### 10.4 Internal routes are mesh-only

**Internal routes (`CAT-I1`, `CAT-I2`) are mesh-only.** Served at `GET /api/v1/internal/departments`/`GET /api/v1/internal/plans` — the same `/api/v1` prefix as every other route in this service, not a separate `/internal/*` URL namespace (corrected in v1.2, **CAT-D8**, §16). The mesh-only boundary is enforced by the `RequireSystemRole` middleware (`iam-system` role check) plus the mesh's NetworkPolicy (IAPI-1) — mTLS-authenticated, no external ingress, no JWT parsing, no tenant context (there is none to check) — not by the path shape.

### 10.5 No PII

Neither table has ever held personal data; nothing about this extraction changes the service's GDPR posture (there is none to have — §15.1).
...
| `GET /api/v1/internal/departments` (CAT-I1) | ≤30 ms p99 | Already stated in §5.4 |
| `GET /api/v1/internal/plans` (CAT-I2) | ≤30 ms p99 | Already stated in §5.4 |
| `POST /api/v1/operator/departments` (CAT-1) / `PATCH .../:id` (CAT-2) | 100 ms p99 | New — single-row write, no downstream synchronous call (§3's leaf-service property) |
...
None of these figures existed in the source LLD for the operator-facing endpoints — O-1/O-2/O-5/O-6 carried no SLO row there either. They are this document's own proposal, sized by analogy to the source LLD's simplest single-row read/write SLOs, and are tracked as an open confirmation item (**CAT-Q5**, §16) rather than presented as already agreed.
...
- `catalog_admin_request_duration_seconds{route}` — a **Histogram** with buckets landing exactly on this section's own SLO thresholds (5/10/15/20/30/40/50/75/100/250/500/1000/2500 ms), not a Summary (reversed from the original design at v1.6, **CAT-D13**, §16): a Summary's client-side quantiles cannot answer "what fraction of requests were under 40 ms" or be aggregated with `histogram_quantile()` across routes, which `deploy/monitoring/slo-rules.yml`'s multi-window burn-rate alerts need. Feeds the SLOs above and that file directly.
- `catalog_admin_writes_total{table, op}` — `departments`/`plans` inserts and updates, split by table (CAT-1/CAT-2 vs. CAT-5).
- `catalog_admin_cache_hits_total{key}` / `catalog_admin_cache_misses_total{key}` — this service's own `cat:departments`/`cat:plans` local cache (§6), **not** a substitute for the consumer-side visibility noted below. Shipped as two counters rather than a single ratio gauge: a hit ratio is a one-line PromQL derivation from two counters already exposed (`sum(rate(catalog_admin_cache_hits_total[5m])) / (sum(rate(catalog_admin_cache_hits_total[5m])) + sum(rate(catalog_admin_cache_misses_total[5m])))`), and instrumenting a third, redundant gauge alongside them would just be another series to keep in sync with the two that already carry the same information — this document previously named only a `catalog_admin_cache_hit_ratio{key}` gauge and was never reconciled to this shipped design.
- `catalog_admin_optimistic_lock_conflicts_total{table}` — CAT-2/CAT-5's `409 optimistic_lock_conflict` rate; a sustained nonzero rate is itself diagnostic of a caller retry-storm or a tooling bug, not expected in steady state given how rare writes are (§2, §9.1).
- HTTP request metrics from the shared Gin middleware, identical shape to every sibling service.
- `pgcommon_*` pool/query/retry instruments from `pgmetrics.InitWithRegisterer` (v1.29), registered onto the same `gincommon.MetricsRegisterer()` as HTTP and `catalog_admin_*` collectors.

**Consumer-side metrics this service does not itself emit, but depends on for full observability:** `om:departments`/`om:plans`/`gm:departments` cache-miss rate and stale-if-error activation count are emitted by Core and Group Mapping Service, not by this service (§11.4 combines both) — flagged explicitly here so a reader does not look for these metrics on this service's own `/metrics` endpoint and conclude they are missing.

### 11.3 Tracing and logging

`gincommon.InitTracingFromEnv()` installs the TracerProvider unconditionally (in-process spans get valid trace IDs even without a collector; OTLP export is a no-op until `OTEL_EXPORTER_OTLP_ENDPOINT` is set), matching `iam-org-membership`/`iam-realm-provisioner`. `pgcommon.Config.Tracer` (`pgadapter.NewOTelTracer`) adds a `db.query` span per query on that same provider. A trace through this service typically shows `inbound.http → core.service.<use_case> → outbound.postgres → outbound.valkey (cat:* read/DEL)`, with no `outbound.*` call to any other IAM service (§3's dependency rule). Structured JSON logs (Zap via `platform-gincommon/pkg/logger`, scraped to Loki) carry `trace_id`, `request_id`, and the `platform_operator` principal's `sub` for every CAT-1/CAT-2/CAT-5 write (audit-adjacent, §7.6). **Every request log line, not just writes, also carries a `tenant_id`** — `IdentityBridgeMiddleware` requires a valid `x-tenant-id` header (401s if it's absent or malformed) and `platform-gincommon`'s shared logging middleware then logs that header, unredacted, as `tenant_id` on every request — even though this service never reads the parsed value for anything afterward (verified: no `.TenantID` reference exists outside the one parse site). See **CAT-D12** (§16) for why the header stays required despite being unused.
...
| Any consumer's stale-if-error fallback activating | SEV-3 (early warning that a consumer's cache is aging past its normal TTL and this service isn't answering, §6) | Not this service's own alert — emitted by Core's/Group Mapping's own monitoring, since the fallback activates in *their* code (§11.4). |
| `410 Gone` traffic on the pre-drop retirement routes during the Wave-1 soak period | SEV-3 (signals a caller that missed the cutover, §19) | Moot — the Wave-1 migration (§19) has completed and the retirement routes this alert would watch were removed from Core entirely in the Contract step, not merely retired-and-kept. This condition was never this service's own alert to ship in the first place (the routes it watches live in Core), and now there is nothing left to watch it for. |
...
| Multi-window error-budget burn on any of §11.1's three route-class latency SLOs | SEV-2 (fast burn, 5m+1h, 14.4× budget) / SEV-3 (slow burn, 1h, 6× budget) | `IAMCatalogAdminSLO{1,2,3}{PublicRead,Internal,Operator}Latency{FastBurn,SlowBurn}` (`deploy/monitoring/slo-rules.yml`) — a proper error-budget burn-rate SLO, distinct from the threshold-only `HighLatency`/`CriticalLatency` alerts below; needs `catalog_admin_request_duration_seconds` to be a Histogram (**CAT-D13**, §16), which is why this alert set could not exist before that reversal. |

This platform's shipped alert labels use `severity: critical`/`severity: warning`, not this document's `SEV-2`/`SEV-3` — the "Shipped as" column states the mapping explicitly (`critical` = SEV-2, `warning` = SEV-3) rather than leaving a reader to infer it. Several more alerts exist in `deploy/monitoring/app-alerts.yml` but are not this table's concern, since none of them map to a condition this section documents — they're the platform-wide conventions every sibling service ships, layered underneath (or alongside) the alerts above, not a substitute for them: the generic, all-route `IAMCatalogAdminHighErrorRate`/`CriticalErrorRate` (5%/20% on `http_requests_total`), `IAMCatalogAdminHighLatency`/`CriticalLatency` (p99 over 250 ms/1 s on `http_request_duration_seconds`), and `IAMCatalogAdminReplicasMissing` (fewer than 2 pods reporting `up` for 5 minutes — an early warning ahead of `IAMCatalogAdminDown`'s full-outage scrape-absence check above). `deploy/monitoring/slo-rules.yml` is a separate, standalone rule file (not part of `prometheusrule.yaml`/`app-alerts.yml`), mirroring `iam-org-membership`'s own `slo-rules.yml` pattern (multi-window multi-burn-rate, SRE book Ch. 5) rather than folding burn-rate recording rules into the simpler threshold-alert files above.

### 11.6 Runbooks

Pointers only here: forcing a consumer cache refresh ahead of its TTL and rotating the Postgres/Valkey connection-string secrets are in §20 (Operational Considerations); the now-closed `410 Gone` migration-soak-recovery and pre-Contract rollback procedures are recorded as historical notes in §19 (Migration Strategy).
...
catalogAdminConfig:
  cache:
    catalogTtlSeconds: 60        # cat:departments / cat:plans (§6)
  migration:
    wave1SoakMinReleases: 1       # minimum releases the 410 Gone soak runs before the Contract step (§19 step 3) — see CAT-Q2, §16
  featureFlags:
    webhookInvalidation: false    # CAT-D3/CAT-D7 — TTL-only at ship time; flip only if the deferred push-based option is built
db:
  pool:
    maxConns: 10                  # sized for a read-heavy, write-rare workload (§2) — no per-tenant connection multiplexing to reason about, unlike every RLS-scoped sibling service
replicas: 2                       # §13.1
```

Secrets: `DATABASE_URL`/`MIGRATION_DATABASE_URL` (RDS Postgres connection strings) and `VALKEY_URL` (the `rediss://` Valkey connection string) — corrected v1.23; this section previously said "none beyond the standard RDS Postgres connection string," omitting `MIGRATION_DATABASE_URL` and `VALKEY_URL`, both of which `values.yaml` secret-manages via the same mechanism. This service has no third-party *credential* to rotate in the sense every other IAM service means by that phrase — no Keycloak Admin API, no external IdP secret (§3) — but it does hold three connection-string secrets, not one, all managed via the platform's existing External Secrets Operator convention, identically to every other IAM service. Configuration is validated at startup, failing fast on an invalid or missing required value, rather than starting in a partially-configured state.
...
Runs in the `iam` namespace alongside its sibling services, at 2 replicas (§11's "sufficient for MVP load" judgment, tracked as an open confirmation item, **CAT-Q5**, §16), behind a Pod Disruption Budget. No public ingress — every route is either `platform_operator`-gated operator tooling or a mesh-internal, `iam-system`-role-gated `/api/v1/internal/*` call (§10); no Envoy route terminates here for ordinary tenant traffic beyond CAT-6/CAT-7's own authenticated-caller reads.

### 13.2 Repository and package layout

`yourorg/catalog-admin-config`, one-repo-per-microservice, following the identical Clean Architecture layout every sibling service uses (§3) — no deviation introduced by this being the smallest service in the stack.

### 13.3 Scaling triggers

| Signal | Action |
|---|---|
| CAT-I1/CAT-I2 request rate exceeds what 2 replicas comfortably serve (unexpected at MVP — every consumer already caches at 600 s, §6) | Add replicas; no code change, since there is no per-tenant state to shard or rebalance (§2 — this service has no concept of "tenant") |
| `cat:departments`/`cat:plans` cache hit ratio drops sustained | Investigate Valkey health before adding replicas — a cache-layer problem, not a compute one, given this service's read pattern is small-payload and highly cacheable by construction (§5.4) |
| Sustained CPU/memory > 70% | Revisit replica count — would itself indicate load far beyond this document's MVP assumptions (§2: "read-heavy, write-rare... writes happen only on operator action") and warrants investigation before simply scaling |

Unlike every sibling service with a tenant-scoped or externally-stateful workload, this service has no sharding, no per-tenant hot spot, and no external-system capacity ceiling to plan around — its scaling story is the simplest in the IAM stack, a direct consequence of §2's "small... service" framing.

**Deliberately no `prometheus-adapter`/custom-metrics HPA config, cross-checked directly against `iam-user-profile/deploy/monitoring/prometheus-adapter-rule.yaml`** (v1.20) — that sibling maps `http_requests_total` to a `http_requests_per_second` custom metric so its HPA can scale on request rate instead of only CPU/memory. This service's own `deploy/helm/templates/hpa.yaml` scales on CPU (70%) and memory (75%) only, matching the table above exactly: request-rate growth is a manual "add replicas" trigger here, not an automated scale target, because at this service's read-heavy/write-rare MVP scale (§2) request rate isn't expected to spike independently of CPU/memory the way it might for a workload with bursty per-tenant traffic. Nothing missing — an RPS-based custom metric would be scope creep against this section's own "simplest scaling story" framing, not a gap.

### 13.4 Environments

Dev, staging, and production follow the platform-wide isolation model — dev/staging as namespaces within the non-production cluster, production as a fully separate cluster. Staging seeds its own copy of the five system departments and three plan tiers independently of production's Wave-1 migration export (§19), so destructive testing (retiring a department, PATCHing a plan tier's limits) in staging can never touch production data — a property worth stating explicitly given how much of this service's own write-path testing (§14) is inherently mutation-of-shared-reference-data by design.

---

## 14. Testing Strategy

This service's testing surface is the simplest in the IAM stack: two tables, no RLS, no external stateful dependency (no Keycloak, no event bus), and a write path that is deliberately rare. The one property worth designing tests around is different from every sibling service's: this document repeatedly claims the extraction is **behavior-preserving** (§19 step 2: "same cached value, new origin"; §4.7's backward-compat table: "byte-identical before/after") — that claim is exactly what the contract-test tier below exists to verify, not just assert.

### 14.1 Unit tests

Table-driven, `core/service` use cases tested against mocked repository interfaces — no network, no containers. Coverage focus: the department lifecycle state machine (§4.2's D-1 through D-11, ported verbatim from the source LLD), the PLAN-6 merge rule as it applies to this service's own write-time validation (rejecting a non-scalar `feature_set` value, §4.6), and the nullable-limit (`NULL = unlimited`) semantics for `workflow_template_limit`/`tender_limit` (§4.2, CAT-D6).

### 14.2 Integration tests

`testcontainers`-based for Postgres, `miniredis`-based for Valkey — no Keycloak, no SNS/SQS, since this service has neither dependency (§3/§7). `miniredis` stands in for Valkey rather than a `testcontainers` Redis module: it's an in-process, real-TCP-server implementation of the Redis wire protocol (`internal/adapter/outbound/valkey/cache_test.go`, `test/e2e/harness_test.go`), not a mock — the cache read-through/invalidation logic is genuinely exercised, just without a container to start. Coverage includes: the full department retirement/reactivation state machine against a real Postgres instance (verifying the DB-layer triggers in §4.5's DDL fire correctly, not just the service-layer checks), optimistic-lock conflict behavior on concurrent CAT-2/CAT-5 writes, and `cat:departments`/`cat:plans` cache population/invalidation against `miniredis`. Every test in this tier (and in the end-to-end tier, §14.4) runs concurrently via `t.Parallel()` — each already provisions its own fully isolated container/pool, so there is no shared state a concurrent run would race on (v1.26).

### 14.3 Contract tests

Two contract surfaces:

- **HTTP contract** (§5): every CAT-1 through CAT-I2 endpoint's request/response shape validated in CI, with a dedicated suite exercising the exact shapes §5.4's documented examples commit to — a break here means Core's or Group Mapping's own cache-population code (which this document asserts will work unmodified against these shapes) is now wrong, which is a cross-team-notification-worthy event, not merely a failing test.
- **Behavior-preservation contract** (specific to this service's extraction claim): a snapshot comparison of `planDefaults(plan)`'s output — computed against Core's pre-cutover local `plans` table and against this service's post-seed `plans` table — asserted byte-identical for all three tiers, directly testing the claim in §4.7/§19 step 2 rather than trusting the seed data was transcribed correctly by inspection alone. Run once, in staging, as a migration-verification gate before the Wave-1 write cutover (§19 step 3) proceeds.

### 14.4 End-to-end tests

A single chain, run against staging: operator creates a custom department (CAT-1) → activates it via Core's `POST /api/v1/tenants/:id/departments` (P-24, outside this service) → operator retires the department (CAT-2) → asserts Core's next `om:departments`-cache-backed activation attempt against it is rejected within the documented TTL window (§6, CAT-D7) — the one end-to-end property this document makes an explicit operational claim about, and therefore the one most worth testing across the service boundary rather than trusting each side's unit tests in isolation.

### 14.5 Migration tests

Every migration in §19 is tested for forward-only application against a snapshot of the previous schema version. The Wave-1 cutover's four stages are each tested independently in staging (per §13.4's isolated staging environment) before being run against production: expand (schema created, seed data matches Core's exported rows exactly), cut-over-reads (Core's/Group Mapping's cache-population code produces identical cached values pre/post, per §14.3's behavior-preservation contract), cut-over-writes (a write to a retired O-1/O-2/O-5/O-6 route returns `410`, never silently succeeds against Core's now-unreplicated copy), and contract (the FK-to-app-check conversion is verified against a deliberately stale cache to confirm the documented fail-safe behavior in §9, before Core's tables are actually dropped).

### 14.6 Chaos and failure-injection tests

Not run as a separate quarterly exercise, unlike a service with an external stateful dependency (Keycloak, an event bus) — this service's entire failure surface is already covered by §9.3's failure matrix and its own straightforward causes (DB down, pod down, cache miss). The one scenario worth deliberately injecting: a CAT-I1/CAT-I2 call failure coinciding with a consumer's primary cache **and** stale-if-error key both being empty (a brand-new Core/Group-Mapping replica's very first request, §9.3) — verified as part of Core's/Group Mapping's own chaos-testing surface, not this service's, since the assertion being tested (does the caller correctly return `503 catalog_unavailable` rather than a wrong answer) lives entirely in the caller's code.

---

## 15. GDPR, Data Lifecycle, and Compliance

### 15.1 What PII actually passes through this service

None. Both `departments` and `plans` are global, operator-managed reference data — a department's `code`/`name` and a plan tier's entitlement columns describe the *catalog*, never a person. The `platform_operator` principal's identity is captured in this service's structured logs (§11.3) for accountability, the same way every operator action is logged platform-wide — this service has no separate audit trail of its own (§7.6, CAT-D10) — but that is provenance metadata about who made an administrative change, not personal data this service holds *about* a data subject. This is stated explicitly, per the same "explicit, documented exemption rather than a silent gap" convention this document already uses for schema governance (§7.4, CAT-EVT-3) — a future reviewer checking why this section is short should find this paragraph, not an unexplained absence.

### 15.2 This service's role in tenant offboarding and GDPR erasure

None. Unlike a service that performs the platform's actual identity-data erasure, or Core Org & Membership and User Profile (each of which scrubs its own tenant-scoped and user-scoped PII rows on offboarding), this service holds no row that references a `tenant_id` or a `user_id` at all (§2.2) — there is structurally nothing for a tenant-offboarding cascade to reach here. A tenant's departments simply stop being *activated* against (their `tenant_departments` rows are handled entirely by Core's own offboarding cascade); the global `departments`/`plans` catalog rows themselves are never touched by any tenant's lifecycle, because they are not that tenant's data in the first place.
...
| Audit-log entries for CAT-1/CAT-2/CAT-5 writes | Per the platform's general-configuration-change retention tier, once this service integrates with the Audit Log Service's direct-write contract (§7.6 — not yet specified anywhere on the platform) | Audit Log Service (not yet integrated — CAT-D10) |
| `cat:departments`/`cat:plans` cache | 60 s TTL, no durable retention (§6) | This service (Valkey) |

No maintenance/pruning job exists for this service (§19.4) — there is no compliance-relevant retention window this document needs to reconcile against a pruning schedule, since nothing here is ever deleted except by an explicit, rare, operator-driven action.

---

## 16. Open Questions and Sign-off Register

Following the same classification convention as the sibling LLDs (Groups **A** — cross-document amendments requiring another document's sign-off, **B** — product/cross-team decisions to confirm, **C** — resolved, recorded for traceability, **D** — deferred, not blocking implementation), with items under the **CAT-Q#** prefix — distinct from this section's own Decision Register subsection (**CAT-D#**, immediately below, which records decisions this document has already made) and from the source LLD's `A#`/`B#`/`C#`/`D#` numbering. None of the items below blocks this document's "Approved for implementation" status — each states explicitly what, if anything, it is still waiting on.
...
`trace_id`/`request_id` are both read via `platform-gincommon` helpers (`gincommon.TraceIDFromContext(c)`/`RequestIDFromContext(c)`, with a header fallback for the latter) in `newErrorResponse` (`internal/adapter/inbound/http/errors.go`) — the same source `HandleError` (`middleware.go`) uses for its own structured log line, so a client's `trace_id` and the corresponding server-side log entry always agree (v1.28 fixed a prior inconsistency where `errors.go` read the trace ID via the raw OTel SDK instead).
...
| `invalid_feature_value` | 400 | `key` (string) | CAT-5's `feature_set` contains a non-scalar value — nested objects/arrays are barred (PLAN-6(d), §4.6). `key` names the offending `feature_set` key. |
...
| `db_unavailable` | 503 | — | A Postgres availability failure — SQLSTATE class `08`/`53` via `pgcommon.IsConnectionException`/`IsInsufficientResources`, class `57`/`58` via `isOperatorOrSystemErrorSQLState`. `wrapConnErr` remaps these to `domain.ErrDBUnavailable` before they reach HTTP; `HandleError` still classifies a leaked PgError the same way without importing `pgconn`. |
...
| `catalog_unavailable` | 503 | — | Emitted by a **consumer** (Core, Group Mapping), not this service — included here because it is the direct downstream consequence of this service's own unavailability coinciding with an empty primary-and-stale-if-error cache (§9.3); a reader of this appendix should not expect to see it in this service's own logs. |
...
This table was missing `duplicate_code` as of v1.3, and — discovered in a later reconciliation pass — six more real, client-facing codes (`validation_error`, `invalid_uuid`, `missing_identity_headers`, `insufficient_role`, `db_unavailable`, `dependency_unavailable`) that `domainErrorStatus` (`internal/adapter/inbound/http/middleware.go`) has mapped all along; this table's own closing claim to be "a single consolidated lookup" wasn't true until now. One declared sentinel is deliberately **not** listed here: `cache_unavailable` is defined in `internal/core/domain/errors.go` and status-mapped in `domainErrorStatus`, but no code path in this service ever constructs it — cache failures are advisory (CAT-FAIL-1) and fall through to Postgres silently, never surfacing as a client-facing error, so including it here would misrepresent it as reachable. **Correction (v1.23):** that last sentence was itself wrong — this table was still missing two real, reachable codes, `unsupported_media_type` (415) and `request_entity_too_large` (413), both added above. No other error code in this table has drifted from what this document's endpoint catalogue (§5.3) and failure matrix (§9.3) already state.

Every code in this table sets the response body's `error` and `code` fields to the same value. `duplicate_code` (and `invalid_uuid`, `parseUUIDParam`'s own validation code, not listed here since it's a shared helper rather than an endpoint-specific error) briefly did not: a handler attaches a more specific sub-code via `WithDetails({"code": ...})` on top of a generic sentinel (`conflict`, `validation_error`), and the shared error-envelope builder (`errorResponseWithDetails`) previously set `error` from the sentinel before that override was merged, never revisiting it — so `error` stayed on the generic sentinel while `code` carried the specific one. Fixed by having `errorResponseWithDetails` re-sync `error` to the final, post-override `code` (`internal/adapter/inbound/http/middleware.go`), so every sub-code, present or future, inherits the guarantee rather than needing a per-call-site fix. See resolved decision **CAT-D9** (§16).

*(Residual documentation note, v1.24, updated v1.32: `router.go`'s body-cap check carries a code comment citing "LLD §20" — this document's error-taxonomy section number under the pre-v1.24 numbering scheme; two structural realignments later (v1.24 to the `iam-lld-user-profile.md` template, v1.32 to the `iam-lld-org-membership-service.md` template) this section is now §17. Left as-is since fixing it is a code-comment change, out of scope for a markdown-only revision; noted here so the citation isn't mistaken for a fresh discrepancy by a future reconciliation pass.)*

---

## 18. Integration Details

This service is a pure leaf in the IAM dependency graph (§3): it makes no synchronous outbound call to any other IAM service (§9.3, CAT-FAIL-4) and holds no outbound client port. Its integration surface is therefore entirely **inbound** — two consumers call it to populate their own local caches. §18.3 collates both edges in one dependency table, mirroring how `iam-lld-org-membership-service.md`'s own §18 is organized.

### 18.1 Integration with `iam-org-membership` (Core)

| Direction | Mechanism | Description |
|---|---|---|
| Core → This | HTTP `GET /api/v1/internal/plans` (CAT-I2) | `planDefaults(plan)` source for Core's I-8 effective-feature merge (§4.6); populated into `om:plans` on a cache miss (600 s TTL, §6.1) |
| Core → This | HTTP `GET /api/v1/internal/departments` (CAT-I1) | Department-catalog validity check for Core's `tenant_departments`/`dept_memberships` write paths, replacing the FKs (`fk_td_department`, `fk_dm_department`) Core lost when these tables physically moved out (§4.7, §19.2); populated into `om:departments` on a miss |

Core is a **read-through client**, not a caller of this service's raw HTTP client from its own use-case code directly — Core's own `port.CatalogAdminClient` is wrapped one layer in by a `service.CatalogService` that adds Core's primary/stale-if-error cache tiers (§6.3) before any business logic ever sees a `Department`/`Plan` value. Neither CAT-I1 nor CAT-I2 sits on Core's I-8 hot-path read (§6.2, §21.1) — `om:plans` is consulted at the projection layer, and `om:departments` gates admin/JIT writes only, never an authorization decision.

### 18.2 Integration with `iam-group-mapping`

| Direction | Mechanism | Description |
|---|---|---|
| Group Mapping → This | HTTP `GET /api/v1/internal/departments` (CAT-I1) | Department-catalog validity for Group Mapping's own department-to-group resolution writes, replacing the FK (`fk_gdm_department`) it lost on the same physical split (§4.7); populated into `gm:departments` on a miss (600 s TTL, §6.1) |

Group Mapping is a Wave-2 service under the same ADR-0007 decomposition as this one (§1) — it shares CAT-I1 with Core rather than needing a separate endpoint, since both consumers want the identical full-catalog snapshot (§5.4).

### 18.3 Dependency table

Every synchronous edge this service participates in, with its latency budget and failure posture — one row per consumer, since neither consumer's call sits on a request path either side treats as latency-critical:

| Contract | Caller → Callee | Path | Latency budget (p99) | Posture on failure | Ref |
|---|---|---|---|---|---|
| `GET /api/v1/internal/plans` (CAT-I2) | Core → This | §18.1 | ≤30 ms | Cache-miss only; warm cache → zero impact on Core; cold + this service down → Core serves `om:plans:stale` (24 h ceiling) | §5.4, §6.3, §9.3 |
| `GET /api/v1/internal/departments` (CAT-I1) | Core → This | §18.1 | ≤30 ms | Cache-miss only; gates Core's admin/JIT writes, never I-8 | §5.4, §9.3 |
| `GET /api/v1/internal/departments` (CAT-I1) | Group Mapping → This | §18.2 | ≤30 ms | Cache-miss only; gates Group Mapping's own JIT writes | §5.4, §9.3 |

**This service has no outbound edge of its own to add to this table** — it calls no other IAM service, publishes no event, and holds no outbound client port (§3, §7). Every row above is this service acting as **callee**; the failure posture each row states is the caller's, not this service's own (this service's own posture as a callee is §9.3's dependency failure matrix, not duplicated here).
...
4. **Contract** — done.
   - `fk_tenants_plan`, `fk_td_department`, `fk_gdm_department` were converted from DB FKs to application-level checks against the `om:departments`/`gm:departments`/`om:plans` caches described in §6, before the table drop, per ADR-0007's ordering.
...
**This service is now Core's sole system of record for both tables, with no fallback on Core's side.** Core's own local `departments`/`plans` tables no longer exist — a `CatalogAdminClient` HTTP call failure coinciding with an empty or expired `om:departments`/`om:plans` cache (past the 24 h stale-if-error ceiling, §6) is now a hard failure for every department/plan-dependent write in Core, not a degraded-but-functional one. This is exactly the failure mode §9's failure matrix already describes (CAT-FAIL-2's "at worst, a stale answer" framing still holds *while the cache has anything in it at all*) — it just was not, until this correction, described as a **live, current** operational fact rather than a hypothetical future state. Rollback past this point requires restoring Core's tables from the pre-drop snapshot/replication stream and replaying any Catalog-Service-only writes since cutover — reversibility ended when step 4 executed, matching the plan's own original rollback note below.
...
- **RLS posture never regresses.** Neither table carries RLS today (§4.3, §10) and none is added — there is no RLS-enabling migration step to get wrong here, unlike every RLS-scoped table in the source LLD.
...
- **`platform_operator` write-gating is a service-layer check, not a migration concern** — no migration in this plan grants or revokes a DB role privilege that changes who may write; §10's role check is unaffected by any step below.

### 19.2 Zero-downtime requirements
...
- **The FK-to-app-check conversion (step 4) is sequenced before, never after, the table drop** — an application-level check against a stale or not-yet-populated cache is a safer failure mode (§9) than a table that no longer exists underneath a live FK.

### 19.4 Maintenance jobs
...
### 19.5 Historical runbook procedures (closed)

Both procedures below applied only during the Wave-1 cutover window and are retained solely as the historical record of what recovery looked like before the Contract step described earlier in this section executed — neither is a currently executable runbook, since the state each one assumes (Core's pre-Contract handlers/tables still live) no longer exists.

**Recovering a stalled `410 Gone` migration soak (closed).** Trigger (historical): the §11.5 "`410 Gone` traffic on the pre-drop retirement routes" alert firing repeatedly past the agreed soak duration (**CAT-Q2**, §16). Procedure (historical): (1) inspect the `410` traffic's source (request logs/tracing, §11.3) to identify the caller that missed the tooling cutover in step 2 above; (2) confirm with that caller's owning team whether it is a known, in-progress migration or a genuinely missed one; (3) if missed, repoint that caller at the new CAT-1 through CAT-7 endpoints and confirm `410` traffic drops to zero before re-evaluating the soak's exit criterion; (4) do not proceed to the Contract step while any `410` traffic remains, regardless of how long the soak has already run. This runbook's window has closed: the Contract step has completed and the `410`-returning routes it would have monitored no longer exist in Core at all (removed, not merely retired) — the alert this runbook's trigger names is itself labeled "moot" in §11.5, for the same reason.

**Rolling back a Wave-1 cutover before the Contract step (closed).** Trigger (historical): a defect discovered in Catalog Service's own behavior after the read or write cutover (steps 2–3 above) but before the Contract step (step 4) had run. Procedure (historical): (1) repoint operator tooling and Core's/Group Mapping's cache-population code back at Core's original endpoints/tables, which remained live and authoritative until Contract ran (the rollback note above); (2) confirm Core's O-1/O-2/O-3/O-5/O-6 handlers were still serving correctly (they only returned `410` post-write-cutover, never removed before Contract); (3) file the defect against this service before attempting cutover again. This rollback path no longer exists: the Contract step has already executed (Core's tables and pre-Contract handlers are gone) — recovery today is via the actual (heavier) post-Contract path described in the rollback note above, restoring Core's tables from the pre-drop snapshot/replication stream and replaying any Catalog-Service-only writes since cutover.

---

## 20. Operational Considerations

### 20.1 Cache and dependency health

This service's own health surface is `/healthz` (liveness, `gincommon.HealthHandler()`) and `/readyz` (readiness — checks Postgres **and** Valkey independently, **CAT-D11**, §16). A `/readyz` failure pulls a pod out of load-balancer rotation even when only Valkey is down, so a sustained `IAMCatalogAdminDown` firing (§11.5) should be triaged by checking both dependencies separately, not assumed to be a Postgres outage by default.

### 20.2 Consumer cache-staleness monitoring

This service does not itself observe `om:departments`/`om:plans`/`gm:departments` cache-miss rates or stale-if-error activation counts — those metrics live in Core's and Group Mapping's own monitoring (§11.2, §11.4's "Consumer Health" panel). Operationally this means a Catalog Service outage can look entirely healthy from this service's own dashboards while a consumer is already degraded onto its stale-if-error fallback (§6.3) — cross-check the consumer-side panel before declaring "all clear" on a reported downstream symptom.

### 20.3 Forcing a consumer cache refresh ahead of its TTL

Trigger: an operator needs a department retirement or a plan-tier edit to take effect for Core/Group Mapping sooner than the 600 s TTL (or the 24 h stale-if-error ceiling) documented in §6/CAT-D7. Procedure: (1) confirm this is genuinely time-sensitive — most retirements are planned and can tolerate the documented window (§6's own guidance); (2) if not, coordinate directly with the Core/Group Mapping on-call to manually evict the specific `om:departments`/`om:plans`/`gm:departments` key via their own operational tooling — this service has no push-invalidation mechanism of its own to do this remotely (CAT-D3, deferred); (3) document the manual evict in the change log, since it is an out-of-band operation this document's normal design does not otherwise require.

### 20.4 Optimistic-lock conflict rate

A sustained nonzero `catalog_admin_optimistic_lock_conflicts_total` rate (§11.2, §11.5's `IAMCatalogAdminOptimisticLockConflicts` alert) is itself diagnostic of a caller retry-storm or a tooling bug, not expected contention — this service's writes are rare enough by design (§2, §9.1) that any sustained conflict rate warrants investigating the calling tool before assuming it reflects normal traffic.

### 20.5 Catalog database credential rotation

This service has no third-party credential (§12) — only connection-string secrets: `DATABASE_URL`, `MIGRATION_DATABASE_URL`, and `VALKEY_URL`. Rotation follows the platform's standard External Secrets Operator rotation hook identically to every sibling service; no service-specific runbook step exists beyond confirming (via §11.4's dashboards) at least one successful query using the new Postgres credential, plus — for `VALKEY_URL` specifically — a `cat:departments`/`cat:plans` cache-hit-ratio panel (§11.2's PromQL derivation) returning non-zero shortly after rotation, before considering rotation complete.

---

## 21. Performance Considerations

### 21.1 Internal bulk-read endpoints (CAT-I1, CAT-I2)

≤30 ms p99 for both `GET /api/v1/internal/departments` and `GET /api/v1/internal/plans` (§5.4) — a small payload (the department catalog is "a few dozen rows"; the plan catalog is fixed at three tiers), a single unindexed `SELECT *` with no joins, and, in steady state, a `cat:departments`/`cat:plans` cache hit (§6.1) rather than a Postgres round trip at all.

### 21.2 Public and operator read endpoints (CAT-4, CAT-6, CAT-7)

15 ms p99 on a `cat:departments`/`cat:plans` cache hit, 40 ms p99 on a miss (§11.1) — CAT-6/CAT-7's `active_only` filter and single-ID lookup are both applied in-memory, after the cache read (§8.4.6, §8.4.7), so neither adds a second query.

### 21.3 Write endpoints (CAT-1, CAT-2, CAT-5)

100 ms p99 (§11.1) — a single-row, single-table `INSERT`/`UPDATE` (`RunInTx`, §9.2) with no downstream synchronous call (this service's leaf-service property, §3) and no multi-statement transaction to serialize on.

### 21.4 No hot path, no sharding

Unlike every tenant-scoped IAM service, this service has no per-tenant hot path to protect and nothing to shard — neither table carries a `tenant_id` (§2.2), so there is no per-tenant connection multiplexing, no per-tenant cache-key fan-out, and no risk of one tenant's traffic pattern degrading another's. §13.3's scaling-trigger table already frames this as "the simplest scaling story in the IAM stack"; this subsection restates the same fact in performance terms rather than operational ones.

### 21.5 Cache sizing

`cat:departments`/`cat:plans`'s 60 s TTL (§6.1) is deliberately short relative to `om:*`/`gm:*`'s 600 s, because this service's own database is the actual source of truth and the local cache exists mainly to shield read replicas from CAT-6/CAT-7 traffic (§6.1) — not to bound propagation delay the way the consumer-side caches do (§6.2). A wider `cat:*` TTL would not meaningfully reduce Postgres load further at this service's read volume (§2), so 60 s is sized to "shield the DB," not to minimize hit-ratio-driven latency.

---

*End of document. This LLD refines Document 1 (`01-hld-delta-decomposition.md`) and `org_membership_lld_5.md`'s `departments`/`plans` sections, and is the input to implementation of the `iam-catalog-admin` service. §1.1 records where each source document is refined; §16 tracks every remaining open item.*
