...
// seed pools share the same connection and drain path as production.
// There is no GUC to inject here (this service has no RLS).
package dbseed
...
// Pool is the seed/assert pool used by postgres/e2e tests. Mirrors
// iam-org-membership / iam-realm-provisioner's test/dbseed so tests never
// open a raw pgxpool beside the production pgcommon.Pool.
type Pool struct {
	inner *pgcommon.Pool
	dsn   string
}
...
// TypeMap satisfies pgx.Rows (added in pgx v5.11.0) — the same *pgtype.Map
// Scan/Values already use internally, so a caller that switches from
// Scan/Values to TypeMap().Scan(...) directly sees identical decoding.
