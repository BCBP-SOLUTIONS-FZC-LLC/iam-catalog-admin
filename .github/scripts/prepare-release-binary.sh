...
#   iam-catalog-admin_{version}_{os}_{arch}[.exe]
#   iam-catalog-admin_{version}_{os}_{arch}[.exe].sha256
#
# The caller (create-github-release.sh) aggregates the .sha256 files into
# a single checksums.txt with `sha256sum iam-catalog-admin_* | grep -v '\.sha256$'`.
set -euo pipefail
...
for target in "${TARGETS[@]}"; do
  ...
  ASSET_NAME="iam-catalog-admin_${RELEASE_TAG}_${GOOS}_${GOARCH}${EXT}"
...
echo "iam-catalog-admin_${RELEASE_TAG}_linux_amd64" > release-asset.name
