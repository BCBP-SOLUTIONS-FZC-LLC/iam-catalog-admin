...
#   docker buildx imagetools inspect ghcr.io/.../iam-catalog-admin:<tag>
MAX_MB=200
size=$(docker image inspect iam-catalog-admin-ci-test --format='{{.Size}}')
mb=$((size / 1024 / 1024))
...
exit_code=0
timeout 10s docker run --rm iam-catalog-admin-ci-test 2>/dev/null || exit_code=$?
...
wait $P2 || {
  echo "::error file=cmd/catalog-admin-config/main.go,title=Startup gate::Server exited 0 on missing env vars — validateRequiredEnv must exit non-zero"
  exit 1
}
