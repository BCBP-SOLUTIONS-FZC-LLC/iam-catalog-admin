...
LLD §13.4 names three dashboards for this service. Only one of them is this
...
- **`catalog-admin-requests-writes.json`** — shipped here. Request
  ...
  (`catalog_admin_*`, LLD §13.2) — no cross-service dependency.
- **Consumer Health** — not shipped here. §13.4 itself says this panel is
...
- **Migration Soak** (Wave-1 only) — not shipped here. §13.4 already
  records this as retired: the Contract step in §12 has completed and the
...
folder alongside the platform's existing "IAM" folder, per §13.4.
